...
class GafasNinosController extends Controller
{
    ...
    private const TIPO = 'gafas_ninos';
    private const GENERO = 'ninos';
    ...
    public function index()
    {
        ...
        return view('admin.gafas_ninos.index', [
            'productos' => $productos,
            'promoImage' => GafasNinosPromoContent::load(),
        ]);
    }
    ...
    public function updatePromoImage(Request $request): RedirectResponse
    {
        ...
        $block = BloqueContenido::query()->firstOrCreate(
            ['clave' => GafasNinosPromoContent::BLOCK_KEY],
            [
                'titulo' => 'Gafas niños: promo de cabecera',
                'cuerpo' => null,
                'datos' => GafasNinosPromoContent::defaults(),
                'esta_activo' => true,
                'orden' => 1,
            ]
        );
        ...
        if ($request->hasFile('promo_image_files')) {
            $this->appendContentBlockImages($block, GafasNinosPromoContent::FIELD_IMAGE, (array) $request->file('promo_image_files'));
        } elseif ($request->hasFile('promo_image_file')) {
            $this->appendContentBlockImages($block, GafasNinosPromoContent::FIELD_IMAGE, [$request->file('promo_image_file')]);
        } elseif ($removePromoImageId > 0) {
            $this->removeContentBlockImageById($block, GafasNinosPromoContent::FIELD_IMAGE, $removePromoImageId);
        } elseif ($removePromoImage) {
            $this->clearContentBlockImages($block, GafasNinosPromoContent::FIELD_IMAGE);
        }

        return redirect()
            ->route('admin.gafas-ninos.index')
            ->with('status', 'Imagen promo de /gafas-niños actualizada.');
    }

    public function create()
    {
        return view('admin.gafas_ninos.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()
            ->route('admin.gafas-ninos.index')
            ->with('status', 'Gafa para niño agregada.');
    }

    public function edit(Producto $producto)
    {
        $this->assertIsGafaNino($producto);

        return view('admin.gafas_ninos.edit', [
            'producto' => $producto,
        ]);
    }
    ...
    {
        return $this->showExcelImportForm('admin.gafas_ninos.import');
    }

    public function importFromExcel(Request $request)
    {
        return $this->importFromExcelGeneric($request, self::TIPO, self::GENERO, 'admin.gafas-ninos.index');
    }

    public function downloadTemplate()
    {
        return $this->downloadExcelTemplate('Plantilla_Gafas_Ninos.xlsx');
    }

    private function assertIsGafaNino(Producto $producto): void
    {
