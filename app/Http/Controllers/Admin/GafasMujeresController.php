...
class GafasMujeresController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($isDescanso) {
            $query->where('genero_objetivo', 'descanso');
        } else {
            $query->whereIn('genero_objetivo', [self::GENERO, 'unisex']);
        }

        $productos = $query->get();

        if ($isDescanso) {
            return view('admin.gafas_descanso.index', [
                'productos' => $productos,
                'promoImage' => GafasDescansoPromoContent::load(),
            ]);
        }

        return view('admin.gafas_mujeres.index', [
            'productos' => $productos,
            'promoImage' => GafasMujeresPromoContent::load(),
        ]);
    }
    ...
    public function updatePromoImage(Request $request): RedirectResponse
    {
        ...
        $isDescanso = $this->isDescansoModule($request);
        $promoService = $isDescanso ? GafasDescansoPromoContent::class : GafasMujeresPromoContent::class;
        $promoTitle = $isDescanso ? 'Gafas deportivas: promo de cabecera' : 'Gafas mujer: promo de cabecera';
        $successMessage = $isDescanso
            ? 'Imagen promo de /gafas-deportivas actualizada.'
            : 'Imagen promo de /gafas-mujeres actualizada.';

        $block = BloqueContenido::query()->firstOrCreate(
            ['clave' => $promoService::BLOCK_KEY],
            [
                'titulo' => $promoTitle,
                'cuerpo' => null,
                'datos' => $promoService::defaults(),
                'esta_activo' => true,
                'orden' => 1,
            ]
        );
        ...
        if ($request->hasFile('promo_image_files')) {
            $this->appendContentBlockImages($block, $promoService::FIELD_IMAGE, (array) $request->file('promo_image_files'));
        } elseif ($request->hasFile('promo_image_file')) {
            $this->appendContentBlockImages($block, $promoService::FIELD_IMAGE, [$request->file('promo_image_file')]);
        } elseif ($removePromoImageId > 0) {
            $this->removeContentBlockImageById($block, $promoService::FIELD_IMAGE, $removePromoImageId);
        } elseif ($removePromoImage) {
            $this->clearContentBlockImages($block, $promoService::FIELD_IMAGE);
        }
    ...
    {
        if ($this->isDescansoModule($request)) {
            return view('admin.gafas_descanso.create');
        }

        return view('admin.gafas_mujeres.create');
    }

    public function store(Request $request)
    {
        $request->merge(['categoria' => $this->moduleCategory($request)]);
        ...
        $validated = $request->validate([
            ...
            'formula_permitida' => ['nullable', 'in:0,1'],
            ...
            'uploaded_color_variant_images' => ['nullable', 'array', 'max:20'],
            'uploaded_color_variant_images.*' => ['nullable', 'file', 'image', 'max:51200'],
            'uploaded_color_variant_images_color' => ['nullable', 'array'],
            'uploaded_color_variant_images_color.*' => ['nullable', 'array'],
            'uploaded_color_variant_images_color.*.*' => ['nullable', 'string', 'max:80'],
        ...
        if ($this->isDescansoModule($request)) {
            $meta['formula_permitida'] = $request->has('formula_permitida')
                ? (bool) ((int) $request->input('formula_permitida'))
                : true;
        }

        $existingColorVariants = $this->normalizeExistingColorVariants($request->input('existing_color_variants', []));
        ...
        $normalColorVariants = array_merge($existingColorVariants, $uploadedColorVariants);
        if ($normalColorVariants !== []) {
            $meta['color_variants'] = $normalColorVariants;
        }
        ...
        return redirect()
            ->route($this->indexRouteName($request))
            ->with('status', $this->isDescansoModule($request) ? 'Gafa deportiva agregada.' : 'Gafa para mujer agregada.');
    }

    public function edit(Request $request, Producto $producto)
    {
        $this->assertIsGafaMujer($producto);

        if ($this->isDescansoModule($request)) {
            abort_unless($producto->genero_objetivo === 'descanso', 404);

            return view('admin.gafas_descanso.edit', [
                'producto' => $producto,
            ]);
        }

        return view('admin.gafas_mujeres.edit', [
            'producto' => $producto,
        ]);
    }
    ...
    $caracteristicas = is_array($producto->caracteristicas) ? $producto->caracteristicas : [];
        ...
        if ($this->isDescansoModule($request)) {
            $meta['formula_permitida'] = $request->has('formula_permitida')
                ? (bool) ((int) $request->input('formula_permitida'))
                : (bool) ($meta['formula_permitida'] ?? true);
        }
        ...
        // --- FUSIONAR IMÁGENES EXISTENTES Y NUEVAS, ELIMINANDO SOLO LAS MARCADAS ---
        ...
        if ($request->has('existing_color_variants')) {
            foreach ($request->input('existing_color_variants') as $variantIndex => $variant) {
                ...
                if (is_array($imagesOrder) && count($imagesOrder) === count($images)) {
                    // images_order es un array con claves = índice, valores = orden
                    $imgWithOrder = [];
                ...
                // Eliminar solo las imágenes marcadas
                ...
                $groupKey = implode('|', array_map(fn (string $colorName) => $this->normalizeColorKey($colorName), $names));
        ...
        $hadExistingVariantPayload = $request->has('existing_color_variants');

        // Fusionar: solo variantes normales (arriba). Camara 3D se guarda aparte.
        // Agrupar imágenes por color (no duplicar bloques)
        $uploadedColorVariants = $this->buildUploadedColorVariants(
            $request,
            true,
            'uploaded_color_variant_images',
        ...
        foreach ($allVariants as $variant) {
            ...
            $groupKey = (string) ($variant['group_key'] ?? implode('|', array_map(fn (string $colorName) => $this->normalizeColorKey($colorName), $variantNames)));
            if ($groupKey === '') {
                $groupKey = 'gris';
                $variantLabel = 'Gris';
            }
            ...
            // Merge images
            $groupedVariants[$groupKey]['images'] = array_values(array_unique(array_merge($groupedVariants[$groupKey]['images'], $variant['images'])));
            // El stock lo toma del último editado
        ...
        } elseif ($hadExistingVariantPayload) {
            unset($meta['color_variants']);
        }

        // Camara 3D: persistencia separada en camera_color_variants
        $uploadedCameraVariants = $this->buildUploadedColorVariants($request, false, 'uploaded_color_images', 'uploaded_color_images_color', 'uploaded_color_images_order', true);
        $existingCameraVariants = [];
        if ($request->has('existing_camera_variants')) {
            foreach ((array) $request->input('existing_camera_variants', []) as $variant) {
                if (!is_array($variant)) {
                    continue;
                }
                ...
                $deleteImages = (array) ($variant['delete_images'] ?? []);

                $filteredImages = [];
                foreach ($images as $img) {
                    $img = trim((string) $img);
                    if ($img === '' || in_array($img, $deleteImages, true)) {
                        continue;
                    }
                    if (!in_array($img, $filteredImages, true)) {
                        $filteredImages[] = $img;
                    }
                }
            ...
            $existingCameraVariants = $this->normalizeExistingColorVariants($meta['camera_color_variants'] ?? []);
        }

        if ($uploadedCameraVariants !== [] || $request->has('existing_camera_variants')) {
            $allCamera = array_merge($existingCameraVariants, $uploadedCameraVariants ?: []);
            $cameraGrouped = [];

            foreach ($allCamera as $variant) {
                $variantNames = $this->normalizeSelectedColorNames($variant['name'] ?? '');
                if ($variantNames === []) {
                    $variantNames = ['Gris'];
                }
                ...
                if (!isset($cameraGrouped[$groupKey])) {
                    $cameraGrouped[$groupKey] = [
                        'name' => $variantLabel,
                        'images' => [],
                    ];
                }

                $cameraGrouped[$groupKey]['images'] = array_values(array_unique(array_merge(
                    $cameraGrouped[$groupKey]['images'],
                    (array) ($variant['images'] ?? [])
                )));
            }

            $meta['camera_color_variants'] = array_values(array_filter($cameraGrouped, static function (array $variant): bool {
                return !empty($variant['images']);
            }));

            if ($meta['camera_color_variants'] === []) {
        ...
        if ($resolvedColor !== '') {
            ...
            // Mantener el color anterior si no se envía uno nuevo
            if (!isset($meta['color'])) {
                unset($meta['color']);
            }
        }
    ...
    {
        if ($this->isDescansoModule($request)) {
            return view('admin.gafas_descanso.import');
        }

        return view('admin.gafas_mujeres.import');
    }

    public function importFromExcel(Request $request)
    {
        return $this->importFromExcelGeneric(
            $request,
            self::TIPO,
            $this->moduleCategory($request),
            $this->indexRouteName($request)
        );
    }

    public function downloadTemplate(Request $request)
    {
        $filename = $this->isDescansoModule($request) 
            ? 'Plantilla_Gafas_Deportivas.xlsx' 
            : 'Plantilla_Gafas_Mujeres.xlsx';

        return $this->downloadExcelTemplate($filename);
    }

    private function isDescansoModule(Request $request): bool
    {
        return $request->routeIs('admin.gafas-descanso.*');
    }

    private function indexRouteName(Request $request): string
    {
        return $this->isDescansoModule($request)
            ? 'admin.gafas-descanso.index'
            : 'admin.gafas-mujeres.index';
    }

    private function moduleCategory(Request $request): string
    {
        return $this->isDescansoModule($request) ? 'descanso' : 'female';
    }

    private function assertIsGafaMujer(Producto $producto): void
    {
