...
class GafasNinasController extends Controller
{
    use Concerns\HandlesProductoImages;
    use Concerns\HandlesGafasExcelImport;

    private const TIPO = 'gafas_ninas';
    private const GENERO = 'ninas';
    ...
    public function index()
    {
        ...
        return view('admin.gafas_ninas.index', [
            'productos' => $productos,
            'promoImage' => GafasNinasPromoContent::load(),
        ]);
    }
    ...
    public function updatePromoImage(Request $request): RedirectResponse
    {
        ...
        $block = BloqueContenido::query()->firstOrCreate(
            ['clave' => GafasNinasPromoContent::BLOCK_KEY],
            [
                'titulo' => 'Gafas niñas: promo de cabecera',
                'cuerpo' => null,
                'datos' => GafasNinasPromoContent::defaults(),
                'esta_activo' => true,
                'orden' => 1,
            ]
        );
        ...
        if ($request->hasFile('promo_image_files')) {
            $this->appendContentBlockImages($block, GafasNinasPromoContent::FIELD_IMAGE, (array) $request->file('promo_image_files'));
        } elseif ($request->hasFile('promo_image_file')) {
            $this->appendContentBlockImages($block, GafasNinasPromoContent::FIELD_IMAGE, [$request->file('promo_image_file')]);
        } elseif ($removePromoImageId > 0) {
            $this->removeContentBlockImageById($block, GafasNinasPromoContent::FIELD_IMAGE, $removePromoImageId);
        } elseif ($removePromoImage) {
            $this->clearContentBlockImages($block, GafasNinasPromoContent::FIELD_IMAGE);
        }

        return redirect()
            ->route('admin.gafas-ninas.index')
            ->with('status', 'Imagen promo de /gafas-niñas actualizada.');
    }

    public function create()
    {
        return view('admin.gafas_ninas.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()
            ->route('admin.gafas-ninas.index')
            ->with('status', 'Gafa para niña agregada.');
    }

    public function edit(Producto $producto)
    {
        $this->assertIsGafaNina($producto);

        return view('admin.gafas_ninas.edit', [
            'producto' => $producto,
        ]);
    }
    ...
    public function update(Request $request, Producto $producto)
    {
        ...
        if ($request->has('existing_color_variants')) {
            foreach ($request->input('existing_color_variants') as $variantIndex => $variant) {
                ...
                $images = array_filter($images, function($img) use ($deleteImages) {
        ...
        // Imágenes nuevas subidas en esta edición
        $uploadedColorVariants = $this->buildUploadedColorVariants(
            $request,
            true,
            'uploaded_color_variant_images',
        ...
        // Fusionar: primero las existentes (ya filtradas), luego las nuevas
    ...
    {
        return $this->showExcelImportForm('admin.gafas_ninas.import');
    }

    public function importFromExcel(Request $request)
    {
        return $this->importFromExcelGeneric($request, self::TIPO, self::GENERO, 'admin.gafas-ninas.index');
    }

    public function downloadTemplate()
    {
        return $this->downloadExcelTemplate('Plantilla_Gafas_Ninas.xlsx');
    }

    private function assertIsGafaNina(Producto $producto): void
    {
