...
class GafasPolarizadasController extends Controller
{
	use Concerns\HandlesProductoImages;
	use Concerns\HandlesGafasExcelImport;

	private const TIPO = 'gafas_polarizadas';
	...
	public function index()
	{
		...
		return view('admin.gafas_polarizadas.index', [
			'productos' => $productos,
			'promoImage' => GafasPolarizadasPromoContent::load(),
		]);
	}
	...
	public function updatePromoImage(Request $request): RedirectResponse
	{
		...
		$block = BloqueContenido::query()->firstOrCreate(
			['clave' => GafasPolarizadasPromoContent::BLOCK_KEY],
			[
				'titulo' => 'Gafas polarizadas: promo de cabecera',
				'cuerpo' => null,
				'datos' => GafasPolarizadasPromoContent::defaults(),
				'esta_activo' => true,
				'orden' => 1,
			]
		);
		...
		if ($request->hasFile('promo_image_files')) {
			$this->appendContentBlockImages($block, GafasPolarizadasPromoContent::FIELD_IMAGE, (array) $request->file('promo_image_files'));
		} elseif ($request->hasFile('promo_image_file')) {
			$this->appendContentBlockImages($block, GafasPolarizadasPromoContent::FIELD_IMAGE, [$request->file('promo_image_file')]);
		} elseif ($removePromoImageId > 0) {
			$this->removeContentBlockImageById($block, GafasPolarizadasPromoContent::FIELD_IMAGE, $removePromoImageId);
		} elseif ($removePromoImage) {
			$this->clearContentBlockImages($block, GafasPolarizadasPromoContent::FIELD_IMAGE);
		}

		return redirect()
			->route('admin.gafas-polarizadas.index')
			->with('status', 'Imagen promo de /gafas-polarizadas actualizada.');
	}

	public function create()
	{
		return view('admin.gafas_polarizadas.create');
	}
	...
	public function store(Request $request)
	{
		$validated = $request->validate([
			...
			'categoria' => ['required', 'in:male,female,unisex,ninos,ninas,gafas_polarizadas,descanso'],
		...
		if ($uploadedColorVariants) {
			...
			if ($existingColorVariants !== []) {
				$meta['color_variants'] = $existingColorVariants;
			}
		}
		...
		$meta['formula_permitida'] = $request->has('formula_permitida') ? (bool)((int)$request->input('formula_permitida')) : true;
		...
		return redirect()
			->route('admin.gafas-polarizadas.index')
			->with('status', 'Gafa polarizada agregada.');
	}

	public function edit(Producto $producto)
	{
		$this->assertIsGafaPolarizada($producto);

		return view('admin.gafas_polarizadas.edit', [
			'producto' => $producto,
		]);
	}
	...
	public function update(Request $request, Producto $producto)
	{
		...
		if ($uploadedColorVariants) {
			...
			if ($existingColorVariants !== []) {
				$meta['color_variants'] = $existingColorVariants;
			} elseif ($request->has('existing_color_variants')) {
		...
		$meta['formula_permitida'] = $request->has('formula_permitida') ? (bool)((int)$request->input('formula_permitida')) : ($meta['formula_permitida'] ?? true);
		...
		$producto->update([
			...
			'genero_objetivo' => $validated['categoria'],
			'material_montura' => $validated['material_montura'] ?? null,
			'color' => $resolvedColor !== '' ? $resolvedColor : ($producto->color ?? null),
			'descripcion' => $validated['descripcion'] ?? null,
			'caracteristicas' => $caracteristicas ?: null,
			'precio' => $precio,
			'precio_oferta' => $precioOferta,
	...
	{
		return $this->showExcelImportForm('admin.gafas_polarizadas.import');
	}

	public function importFromExcel(Request $request)
	{
		return $this->importFromExcelGeneric($request, self::TIPO, 'gafas_polarizadas', 'admin.gafas-polarizadas.index');
	}

	public function downloadTemplate()
	{
		return $this->downloadExcelTemplate('Plantilla_Gafas_Polarizadas.xlsx');
	}

	private function assertIsGafaPolarizada(Producto $producto): void
	{
