<?php $__env->startSection('title', 'Gafas hombre'); ?>
<?php $__env->startSection('heading', 'Gafas hombre'); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    <div class="mb-6 rounded-3xl border border-zinc-200 bg-white p-5">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <h3 class="text-base font-semibold text-zinc-900">Imagen promo de hombre en /gafas</h3>
                <p class="mt-1 text-sm text-zinc-500">Se mostrará cuando entren a /gafas con el filtro Hombre.</p>
            </div>
        </div>

        <form class="mt-4" action="<?php echo e(route('admin.gafas-hombre.promo-image.update')); ?>" method="POST" enctype="multipart/form-data">
            <?php echo csrf_field(); ?>
            <div class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
                <div>
                    <label for="gafasHombrePromoImage" class="text-sm font-semibold text-zinc-800">Subir nuevas imágenes</label>
                    <input id="gafasHombrePromoImage" name="promo_image_files[]" type="file" accept="image/*" multiple class="mt-2 block w-full rounded-2xl border border-zinc-200 px-4 py-3 text-sm file:mr-4 file:rounded-xl file:border-0 file:bg-zinc-900 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white hover:file:bg-zinc-800">
                    ...
                    <?php endif; ?>
                    <p class="mt-1 text-xs text-zinc-500">Las imágenes subidas se intercalan automáticamente en /gafas cuando el filtro Hombre está activo.</p>
                ...
                <div class="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50">
                    <img src="<?php echo e($promoPreview); ?>" alt="Vista previa banner hombre" class="h-56 w-full object-cover" loading="lazy">
                </div>
            </div>

            <?php if($promoAssets !== []): ?>
                <div class="mt-4">
                    <p class="text-xs font-semibold uppercase tracking-wide text-zinc-500">Vista previa banners hombre</p>
                    <div class="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
                        <?php $__currentLoopData = $promoAssets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $promoAsset): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <div class="relative overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50">
                                ...
                                <?php endif; ?>
                                <img src="<?php echo e($promoImageUrl); ?>" alt="Banner hombre" class="h-24 w-full object-cover" loading="lazy">
                            </div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-sm text-zinc-500">Administra tus gafas para hombre y sus imágenes.</p>
        </div>
        <div class="flex flex-wrap gap-3">
            <a href="<?php echo e(route('admin.gafas-hombre.import.show')); ?>" class="inline-flex items-center justify-center rounded-2xl border border-zinc-200 bg-white px-4 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">
                Importar Excel
            </a>
            <a href="<?php echo e(route('admin.gafas-hombre.create')); ?>" class="inline-flex items-center justify-center rounded-2xl bg-zinc-950 px-4 py-2 text-sm font-semibold text-white hover:bg-zinc-900">
                + Agregar
            </a>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[720px] text-left text-sm">
                    ...
                    <tr class="bg-white">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <a href="<?php echo e(route('admin.gafas-hombre.edit', $producto)); ?>" class="rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">Editar</a>
                                <form action="<?php echo e(route('admin.gafas-hombre.destroy', $producto)); ?>" method="POST" onsubmit="return confirm('¿Eliminar esta gafa?')">
                                    <?php echo csrf_field(); ?>
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr>
                        <td colspan="4" class="px-5 py-10 text-center text-sm text-zinc-500">Aún no has agregado gafas para hombre.</td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
    ...
    <div class="mt-6">
        <a href="<?php echo e(route('gafas-hombre.index')); ?>" class="text-sm font-semibold text-zinc-700 hover:underline">Ver página pública: /gafas-hombre</a>
    </div>

    <?php echo $__env->make('admin.partials.auto_refresh', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.dashboard_empty_sidebar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\norla\Desktop\Proyectos con Ronaldo\copia_optica\resources\views/admin/gafas_hombre/index.blade.php ENDPATH**/ ?>