<?php $__env->startSection('title', 'Gafas polarizadas'); ?>
<?php $__env->startSection('heading', 'Gafas polarizadas'); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    <div class="mb-6 rounded-3xl border border-zinc-200 bg-white p-5">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <h3 class="text-base font-semibold text-zinc-900">Imagen promo de polarizadas en /gafas</h3>
                <p class="mt-1 text-sm text-zinc-500">Se mostrará cuando entren a /gafas con el filtro Polarizadas.</p>
            </div>
        </div>

        <form class="mt-4" action="<?php echo e(route('admin.gafas-polarizadas.promo-image.update')); ?>" method="POST" enctype="multipart/form-data">
            <?php echo csrf_field(); ?>
            <div class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
                <div>
                    <label for="gafasPolarizadasPromoImage" class="text-sm font-semibold text-zinc-800">Subir nuevas imágenes</label>
                    <input id="gafasPolarizadasPromoImage" name="promo_image_files[]" type="file" accept="image/*" multiple class="mt-2 block w-full rounded-2xl border border-zinc-200 px-4 py-3 text-sm file:mr-4 file:rounded-xl file:border-0 file:bg-zinc-900 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white hover:file:bg-zinc-800">
                    ...
                    <?php endif; ?>
                    <p class="mt-1 text-xs text-zinc-500">Las imágenes subidas se intercalan automáticamente en /gafas cuando el filtro Polarizadas está activo.</p>
                ...
                <div class="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50">
                    <img src="<?php echo e($promoPreview); ?>" alt="Vista previa banner polarizadas" class="h-56 w-full object-cover" loading="lazy">
                </div>
            </div>

            <?php if($promoAssets !== []): ?>
                <div class="mt-4">
                    <p class="text-xs font-semibold uppercase tracking-wide text-zinc-500">Vista previa banners polarizadas</p>
                    <div class="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
                        <?php $__currentLoopData = $promoAssets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $promoAsset): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <div class="relative overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50">
                                ...
                                <?php endif; ?>
                                <img src="<?php echo e($promoImageUrl); ?>" alt="Banner polarizadas" class="h-24 w-full object-cover" loading="lazy">
                            </div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-sm text-zinc-500">Administra tus gafas polarizadas y sus imágenes.</p>
        </div>
        <div class="flex flex-wrap gap-3">
            <a href="<?php echo e(route('admin.gafas-polarizadas.import.show')); ?>" class="inline-flex items-center justify-center rounded-2xl border border-zinc-200 bg-white px-4 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">
                Importar Excel
            </a>
            <a href="<?php echo e(route('admin.gafas-polarizadas.create')); ?>" class="inline-flex items-center justify-center rounded-2xl bg-zinc-950 px-4 py-2 text-sm font-semibold text-white hover:bg-zinc-900">
                + Agregar
            </a>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[720px] text-left text-sm">
                    ...
                    <tr class="bg-white">
                        ...
                        <td class="px-5 py-4">
                            ...
                            <?php if($producto->esta_activo): ?>
                                <span class="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200/80">Activo</span>
                            <?php else: ?>
                                <span class="inline-flex items-center rounded-full bg-zinc-50 px-2.5 py-1 text-xs font-semibold text-zinc-700 ring-1 ring-zinc-200/80">Borrador</span>
                            <?php endif; ?>
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="inline-flex items-center gap-2">
                                <a href="<?php echo e(route('admin.gafas-polarizadas.edit', $producto)); ?>" class="rounded-xl px-3 py-1.5 text-xs font-semibold text-zinc-700 hover:bg-zinc-50">Editar</a>
                                <form action="<?php echo e(route('admin.gafas-polarizadas.destroy', $producto)); ?>" method="POST" onsubmit="return confirm('¿Seguro que deseas eliminar esta gafa?');">
                                    <?php echo csrf_field(); ?>
                                    <?php echo method_field('DELETE'); ?>
                                    <button type="submit" class="rounded-xl px-3 py-1.5 text-xs font-semibold text-rose-600 hover:bg-rose-50">Eliminar</button>
                                </form>
                            </div>
                        </td>
                    </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr>
                        <td colspan="4" class="px-5 py-6 text-center text-sm text-zinc-500">Aún no hay gafas polarizadas.</td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
    ...
    <div class="mt-6">
        <a href="<?php echo e(route('gafas-polarizadas.index')); ?>" class="text-sm font-semibold text-zinc-700 hover:underline">Ver página pública: /gafas-polarizadas</a>
    </div>

    <?php echo $__env->make('admin.partials.auto_refresh', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.dashboard_empty_sidebar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\norla\Desktop\Proyectos con Ronaldo\copia_optica\resources\views/admin/gafas_polarizadas/index.blade.php ENDPATH**/ ?>