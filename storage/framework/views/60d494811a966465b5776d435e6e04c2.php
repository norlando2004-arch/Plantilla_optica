...
<?php $__env->startSection('content'); ?>
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight">Hero banners</h1>
            <p class="mt-1 text-sm text-zinc-600">Se muestran en el carrusel de la landing. Puedes crear todos los que quieras.</p>
        </div>

        <a href="<?php echo e(route('configuracion.hero-banners.create')); ?>" class="inline-flex items-center justify-center rounded-2xl bg-zinc-900 px-5 py-3 text-sm font-semibold text-white hover:bg-zinc-800">Nuevo banner</a>
    </div>

    <form method="POST" action="<?php echo e(route('configuracion.hero-banners.settings.update')); ?>" class="mt-4 rounded-2xl border border-zinc-200 bg-white p-4">
        <?php echo csrf_field(); ?>
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <label for="seconds_per_slide" class="text-sm font-semibold text-zinc-800">Cambiar imagen cada (segundos)</label>
                <input id="seconds_per_slide" name="seconds_per_slide" type="number" min="2" max="30" value="<?php echo e(old('seconds_per_slide', (int)($heroCarousel['seconds_per_slide'] ?? 5))); ?>" class="mt-2 w-32 rounded-xl border border-zinc-300 px-3 py-2 text-sm">
            </div>
    ...
    <div class="mt-6 overflow-hidden rounded-3xl border border-zinc-200 bg-white">
        <table class="min-w-full divide-y divide-zinc-200 text-sm">
            ...
            <?php $__empty_1 = true; $__currentLoopData = $banners; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $banner): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php ($imgUrl = $banner->heroImageUrl(asset('images/placeholder.svg'))); ?>
                ...
                <tr>
                    ...
                    <td class="px-4 py-3">
                        ...
                        <p class="mt-2 text-xs text-zinc-500"><?php echo e($banner->uploaded_image_original_name ?: 'Imagen de banner'); ?></p>
                    </td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex justify-end gap-2">
                            <a href="<?php echo e(route('configuracion.hero-banners.edit', $banner)); ?>" class="rounded-xl px-3 py-2 text-sm font-semibold text-zinc-700 ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50">Editar</a>
                            <form method="POST" action="<?php echo e(route('configuracion.hero-banners.destroy', $banner)); ?>" onsubmit="return confirm('¿Eliminar este banner?');">
                                <?php echo csrf_field(); ?>
            ...
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                <tr>
                    <td colspan="5" class="px-4 py-10 text-center text-sm text-zinc-600">
                        Aún no hay banners. La landing mostrará la imagen por defecto.
                    </td>
                </tr>
            <?php endif; ?>
            </tbody>
