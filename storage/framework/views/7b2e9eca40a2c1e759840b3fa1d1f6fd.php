...
<div class="relative z-[70] bg-white px-3 py-1.5 text-center text-[13px] font-semibold text-zinc-500 md:text-[14px] tracking-wide border-b border-zinc-100">
    coloca tu texto
</div>

<header class="relative z-[70] border-b border-zinc-200 bg-white text-zinc-900 shadow-sm">
    <div class="w-full px-2 py-2 md:px-3">
        <div class="flex items-center gap-2 md:gap-4">
            <a href="<?php echo e($landingUrl); ?>" class="shrink-0" aria-label="Inicio Optica">
                <img
                    src="<?php echo e(asset('images/navbar.png')); ?>"
                    alt="Optica"
            ...
            <nav class="ml-2 hidden flex-1 items-center justify-center gap-3 text-[14px] font-semibold md:ml-4 md:flex md:gap-6 md:text-[17px] md:leading-tight">
                <div class="relative" id="js-store-gafas-menu-wrap">
                    <button
                        ...
                        type="button"
                        class="inline-flex items-center gap-1 whitespace-nowrap px-1 py-1 text-zinc-900 hover:text-zinc-500"
                        aria-haspopup="true"
                        aria-expanded="false"
                    ...
                    <div id="js-store-gafas-menu-panel" class="pointer-events-none absolute left-0 top-[calc(100%+0.5rem)] z-[85] w-56 scale-95 rounded-2xl border border-zinc-200 bg-white p-2 opacity-0 shadow-[0_18px_35px_-18px_rgba(0,0,0,0.45)] transition-all duration-200" aria-hidden="true">
                        ...
                        <?php $__currentLoopData = $gafasDropdownOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <a href="<?php echo e($option['href']); ?>" class="flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm font-semibold text-zinc-800 transition-colors hover:bg-zinc-100">
                                <img src="<?php echo e(asset('images/BotonCategoria.png')); ?>" alt="" class="h-4 w-4 shrink-0" loading="lazy" decoding="async" aria-hidden="true">
                                <span><?php echo e($option['label']); ?></span>
                            </a>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                </div>
                <a href="<?php echo e(url('/gafas-polarizadas')); ?>" class="whitespace-nowrap px-1 py-1 text-zinc-900 hover:text-zinc-500">Gafas de sol</a>
                <a href="<?php echo e($helpUrl); ?>" data-force-nav="1" class="whitespace-nowrap px-1 py-1 text-zinc-900 hover:text-zinc-500">Gafas deportivas</a>
                <a
                    href="<?php echo e($aboutUrl); ?>"
                    data-about-link="1"
                    class="whitespace-nowrap px-1 py-1 text-zinc-900 hover:text-zinc-500"
                    <?php if($aboutOpensNewTab): ?> target="_blank" rel="noopener noreferrer" <?php endif; ?>
                >Sobre nosotros</a>
            </nav>

            <div class="ml-auto flex items-center gap-1 md:ml-3 md:gap-2">
                <?php if($storeNavbarIsAdmin): ?>
                    <div class="relative hidden md:block" id="js-store-admin-menu-wrap">
                        <button
                            id="js-store-admin-menu-toggle"
                            type="button"
                            class="inline-flex h-9 items-center gap-1.5 rounded-full border border-zinc-300 bg-zinc-100 px-3.5 text-[12px] font-extrabold tracking-[0.02em] text-zinc-700 transition-all duration-200 hover:-translate-y-[1px] hover:bg-zinc-200"
                            aria-haspopup="true"
                            aria-expanded="false"
                            aria-controls="js-store-admin-menu-panel"
                        >
                            <span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-zinc-800 text-[10px] font-black text-white">A</span>
                            <span>Admin</span>
                            <svg data-admin-chevron viewBox="0 0 24 24" class="h-4 w-4 text-zinc-500 transition-transform duration-200" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                <path d="m6 9 6 6 6-6"></path>
                            </svg>
                        </button>

                        <div id="js-store-admin-menu-panel" class="pointer-events-none absolute right-0 top-[calc(100%+0.55rem)] z-[85] w-52 scale-95 rounded-2xl border border-zinc-200 bg-white/95 p-2 opacity-0 shadow-[0_18px_35px_-18px_rgba(0,0,0,0.25)] backdrop-blur-sm transition-all duration-200" aria-hidden="true">
                            <p class="px-3 pb-1 pt-0.5 text-[10px] font-black uppercase tracking-[0.16em] text-zinc-400">Panel rapido</p>
                            <?php if($storeNavbarHasConfigRoute): ?>
                                <a href="<?php echo e(route('configuracion.index')); ?>" class="block rounded-xl px-3 py-2 text-sm font-semibold text-zinc-700 transition-colors hover:bg-zinc-50 hover:text-zinc-900">Configuracion</a>
                            <?php endif; ?>
                            <?php if($storeNavbarHasDashboardRoute): ?>
                                <a href="<?php echo e(route('dashboard')); ?>" class="block rounded-xl px-3 py-2 text-sm font-semibold text-zinc-700 transition-colors hover:bg-zinc-50 hover:text-zinc-900">Dashboard</a>
                            <?php endif; ?>
                            <?php if(in_array($storeNavbarRoleId, [3, 4], true) && $storeNavbarHasAdminRoute): ?>
                                <a href="<?php echo e(route('admin')); ?>" class="block rounded-xl px-3 py-2 text-sm font-semibold text-zinc-700 transition-colors hover:bg-zinc-50 hover:text-zinc-900">Admin</a>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endif; ?>

                <button type="button" data-open-store-search="1" class="hidden h-9 w-9 items-center justify-center rounded-full text-zinc-700 hover:bg-zinc-100 md:inline-flex" aria-label="Buscar">
                ...
                <a href="<?php echo e($favoritesUrl); ?>" class="hidden h-9 w-9 items-center justify-center rounded-full text-zinc-700 hover:bg-zinc-100 md:inline-flex" aria-label="Favoritos">
                    <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                ...
                <a href="<?php echo e($trackingUrl); ?>" class="hidden h-9 w-9 items-center justify-center rounded-full text-zinc-700 hover:bg-zinc-100 md:inline-flex" aria-label="Seguimiento de pedido">
                    <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                ...
                <button id="js-store-mobile-menu-open" type="button" class="inline-flex h-9 w-9 items-center justify-center rounded-full text-zinc-700 hover:bg-zinc-100 md:hidden" aria-label="Abrir menú" aria-controls="js-store-mobile-menu" aria-expanded="false">
        ...
        <nav class="mt-5 grid gap-2 text-[15px]">
            <div class="rounded-xl border border-zinc-200 px-2 py-2">
                ...
                <div id="js-store-mobile-gafas-panel" class="mt-1 hidden gap-1.5 pl-1 pr-1 pb-1">
                    <?php $__currentLoopData = $gafasDropdownOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="<?php echo e($option['href']); ?>" class="flex items-center gap-2.5 rounded-lg border border-zinc-200 px-3 py-2 text-sm font-semibold text-zinc-800 hover:bg-zinc-50">
                            <img src="<?php echo e(asset('images/BotonCategoria.png')); ?>" alt="" class="h-4 w-4 shrink-0" loading="lazy" decoding="async" aria-hidden="true">
                            <span><?php echo e($option['label']); ?></span>
                        </a>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            ...
            <a href="<?php echo e(url('/gafas-polarizadas')); ?>" class="rounded-xl border border-zinc-200 px-4 py-3 font-semibold text-zinc-900 hover:bg-zinc-50">Gafas de sol</a>
            <a href="<?php echo e($helpUrl); ?>" data-force-nav="1" class="rounded-xl border border-zinc-200 px-4 py-3 font-semibold text-zinc-900 hover:bg-zinc-50">Gafas deportivas</a>
            <a
                href="<?php echo e($aboutUrl); ?>"
                data-about-link="1"
            ...
            <?php if($storeNavbarIsAdmin): ?>
                <div class="rounded-2xl border border-zinc-200 bg-zinc-50 px-3 py-2">
                    <button id="js-store-mobile-admin-toggle" type="button" class="flex w-full items-center justify-between rounded-xl px-2 py-2 text-left font-extrabold tracking-[0.02em] text-zinc-700 hover:bg-white" aria-expanded="false" aria-controls="js-store-mobile-admin-panel">
                        <span class="inline-flex items-center gap-2">
                            <span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-zinc-700 text-[10px] font-black text-white">A</span>
                            <span>Admin</span>
                        </span>
                        <svg viewBox="0 0 24 24" class="h-4 w-4 text-zinc-500 transition-transform duration-200" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                            <path d="m6 9 6 6 6-6"></path>
                        </svg>
                    </button>
                    <div id="js-store-mobile-admin-panel" class="mt-2 hidden gap-2 pl-2">
                        <?php if($storeNavbarHasConfigRoute): ?>
                            <a href="<?php echo e(route('configuracion.index')); ?>" class="block rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">Configuracion</a>
                        <?php endif; ?>
                        <?php if($storeNavbarHasDashboardRoute): ?>
                            <a href="<?php echo e(route('dashboard')); ?>" class="block rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">Dashboard</a>
                        <?php endif; ?>
                        <?php if(in_array($storeNavbarRoleId, [3, 4], true) && $storeNavbarHasAdminRoute): ?>
                            <a href="<?php echo e(route('admin')); ?>" class="block rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">Admin</a>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endif; ?>
            ...
            <a href="<?php echo e($favoritesUrl); ?>" class="flex items-center justify-between rounded-xl border border-zinc-200 px-4 py-3 font-semibold text-zinc-900 hover:bg-zinc-50">
                <span>Favoritos</span>
                <svg viewBox="0 0 24 24" class="h-5 w-5 text-zinc-700" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                    <path d="M12 20.5s-6.8-4.3-9-8.6c-1.9-3.7.3-6.9 3.7-6.9 2 0 3.6 1.1 5.3 3 1.7-1.9 3.3-3 5.3-3 3.4 0 5.6 3.2 3.7 6.9-2.2 4.3-9 8.6-9 8.6z"></path>
                </svg>
            ...
            <a href="<?php echo e($trackingUrl); ?>" class="flex items-center justify-between rounded-xl border border-zinc-200 px-4 py-3 font-semibold text-zinc-900 hover:bg-zinc-50">
...
?>
<?php if(($showStoreBanner ?? true) && !$hideStoreBannerForRoute): ?>
    <section aria-label="Banner principal" class="w-full">
        ...
        ?>
        <?php if(count($resolvedStoreBannerImages) > 1): ?>
            <style>
            ...
            <?php if($storeNavbarIsGafasListingRoute): ?>
                <style>
                    .store-banner-carousel {
            ...
            <div class="store-banner-carousel" data-store-banner-carousel="1">
                <div class="store-banner-carousel-track" data-store-banner-track="1">
                    <?php $__currentLoopData = $resolvedStoreBannerImages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $bannerImage): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="store-banner-carousel-slide" data-store-banner-slide="1">
                            <img
                                ...
                                fetchpriority="<?php echo e($loop->first ? 'high' : 'low'); ?>"
                                decoding="async"
                            >
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <?php else: ?>
            <div class="store-banner-image-wrap">
                <img
                    src="<?php echo e($resolvedStoreBannerImage); ?>"
                    alt="Banner principal"
        ...
        <div class="relative mx-auto flex min-h-dvh max-w-lg items-center justify-center px-4">
            <div class="w-full rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
                ...
                <div class="mt-5 grid gap-3 sm:grid-cols-2">
                    <a href="<?php echo e($loginUrl); ?>" class="inline-flex items-center justify-center rounded-2xl bg-zinc-950 px-4 py-3 text-sm font-semibold text-white hover:bg-zinc-900 focus:outline-none focus:ring-2 focus:ring-zinc-400">Iniciar sesión</a>
                    <a href="<?php echo e(route('register.show')); ?>" class="inline-flex items-center justify-center rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm font-semibold text-zinc-900 hover:bg-zinc-50">Registrarme</a>
                </div>
            </div>
        </div>
    </div>
...
<?php echo $__env->make('partials.store_search_modal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

<script>
(() => {
    const isAuthed = Boolean(<?php echo json_encode(\Illuminate\Support\Facades\Auth::check(), 15, 512) ?>);
...
<?php /**PATH C:\Users\norla\Desktop\Proyectos con Ronaldo\copia_optica\resources\views/partials/store-navbar.blade.php ENDPATH**/ ?>