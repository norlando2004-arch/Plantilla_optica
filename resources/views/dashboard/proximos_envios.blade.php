...
@section('heading', 'Próximos envíos')
...
@section('content')
    <div class="rounded-3xl border border-zinc-200 bg-gradient-to-br from-zinc-50 to-white p-6 shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="inline-flex items-center rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-zinc-700 ring-1 ring-zinc-200">
                        {{ $pagos->count() }} {{ $pagos->count() === 1 ? 'pedido' : 'pedidos' }}
                    </span>
                </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('dashboard.proximos-envios', ['view' => 'pendientes']) }}" class="inline-flex items-center justify-center rounded-2xl px-3 py-2 text-xs font-semibold {{ ($view ?? 'pendientes') === 'pendientes' ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-800 border border-zinc-200 hover:bg-zinc-100' }}">
                    Próximos envíos
                </a>
                <a href="{{ route('dashboard.proximos-envios', ['view' => 'enviadas']) }}" class="inline-flex items-center justify-center rounded-2xl px-3 py-2 text-xs font-semibold {{ ($view ?? 'pendientes') === 'enviadas' ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-800 border border-zinc-200 hover:bg-zinc-100' }}">
                    Ver enviadas
                </a>
            </div>
        </div>

        @if(($view ?? 'pendientes') === 'enviadas')
            <form method="GET" action="{{ route('dashboard.proximos-envios') }}" class="mt-4 flex flex-wrap items-end gap-3">
                <input type="hidden" name="view" value="enviadas" />
                <div>
                    <label class="text-xs font-semibold text-zinc-700">Buscar por cédula</label>
                    <input name="cedula" value="{{ $cedula ?? '' }}" class="mt-1 w-48 rounded-2xl border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:border-zinc-400" placeholder="Número de documento" />
                </div>
                ...
                @foreach($pagos as $pago)
                    ...
                    <div class="overflow-hidden rounded-[28px] border border-zinc-200 bg-white shadow-[0_14px_35px_-24px_rgba(15,23,42,0.32)]">
                        <div class="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-200 bg-[linear-gradient(120deg,#fbfdff_0%,#eef4fb_100%)] px-5 py-4">
                            <div>
                                <p class="text-lg font-semibold text-zinc-900">Referencia: {{ $pago->referencia }}</p>
                                <div class="mt-1 flex flex-wrap items-center gap-2 text-sm text-zinc-600">
                                    <span>Aprobado • {{ $approvedAtLabel ?? '—' }}</span>
                                    <span class="inline-flex items-center rounded-full bg-sky-50 px-2.5 py-0.5 text-[11px] font-semibold text-sky-700">{{ $perfil ? 'Cliente registrado' : 'Compra invitada' }}</span>
                                    @if($pago->envio_estado === 'enviado')
                            ...
                            <div class="ml-auto flex items-center gap-3">
                                <div class="min-w-[150px] text-right">
                                    ...
                                    <p class="text-[42px] leading-none font-bold text-blue-700">{{ number_format((float) $pago->monto, 0, ',', '.') }} {{ $pago->moneda }}</p>
                                </div>

                                <div class="grid gap-2">
                                    <a href="{{ route('pagos.show', $pago) }}" class="inline-flex items-center justify-center rounded-full bg-blue-700 px-5 py-2 text-sm font-semibold text-white hover:bg-blue-800">
                                        Ver pago
                                    </a>
                                    @if($prescriptionId)
                                        <a href="{{ route('dashboard.pagos.formula', ['pago' => $pago->id]) }}" target="_blank" rel="noopener" class="inline-flex items-center justify-center rounded-full border border-zinc-200 bg-white px-5 py-2 text-xs font-semibold text-zinc-700 hover:bg-zinc-50">
                                            Ver fórmula
                                        </a>
                                    @endif
                                    ...
                                    @else
                                        <div class="grid gap-2">
                                                ...
                                                <button type="button" data-toggle-form="upload-form-{{ $pago->id }}" class="inline-flex items-center justify-center rounded-full px-5 py-2 text-sm font-semibold {{ $buttonDisabled ? 'bg-zinc-100 text-zinc-400 border border-zinc-200 cursor-not-allowed opacity-60' : 'border border-blue-300 bg-white text-blue-700 hover:bg-blue-50' }}" {{ $buttonDisabled ? 'disabled title="Ya existe un comprobante subido"' : '' }}>
                                                    Marcar enviado / Subir comprobante
                                                </button>

                                            <div id="upload-form-{{ $pago->id }}" style="display:none;" class="rounded-2xl border border-zinc-200 bg-white p-3">
                                                @if($shippingFile)
                                                    <p class="text-sm text-zinc-700">Ya existe un archivo subido: <strong>{{ $shippingFile['original_name'] ?? basename($shippingFile['path'] ?? '') }}</strong></p>
                                                    <p class="text-sm text-zinc-600">Tipo: {{ $shippingFile['type'] ?? '—' }}. Para cambiarlo, elimina el existente primero.</p>
                                                @else
                                                    <form method="POST" action="{{ route('dashboard.proximos-envios.upload-shipping', $pago) }}" enctype="multipart/form-data">
                                                        @csrf
                            ...
                            <div class="grid gap-4 lg:grid-cols-12">
                                <article class="rounded-3xl border border-zinc-200 bg-zinc-50 p-5 lg:col-span-4">
                                    ...
                                    <div class="mt-3 flex items-start justify-between gap-3">
                                        <h3 class="text-4xl leading-[1.1] font-semibold text-zinc-900">{{ $productTitle }}</h3>
                                    ...
                                    <p class="mt-3 text-sm text-zinc-700"><span class="font-semibold text-zinc-900">Especificaciones:</span> {{ $productSpecsLine }}</p>

                                    @if($frameColor !== '')
                                        <p class="mt-2 text-sm text-zinc-700"><span class="font-semibold text-zinc-900">Color de montura:</span> {{ $frameColor }}</p>
                                    @endif

                                    <div class="mt-4 grid grid-cols-2 gap-3">
                                        @foreach($productMeasureItems as $measure)
                                            <div class="flex items-start gap-2 rounded-2xl bg-white px-3 py-2.5 ring-1 ring-zinc-200">
                                                ...
                                                <div>
                                                    <p class="text-sm text-zinc-600">{{ $measure['label'] }}</p>
                                                    <p class="text-3xl font-semibold text-zinc-900 leading-none">{{ $measure['value'] }}</p>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                    ...
                                    <div class="mt-4 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
                                        <div class="space-y-2 lg:border-r lg:border-zinc-200 lg:pr-4">
                                            <div class="flex items-start justify-between gap-3 py-1">
                                                ...
                                                <span class="text-right text-zinc-800">{{ $buyer['nombre'] ?? ($usuario->nombre ?? ($guest['nombre'] ?? '—')) }}</span>
                                            </div>
                                            <div class="flex items-start justify-between gap-3 py-1">
                                                <span class="font-semibold text-zinc-900">Correo</span>
                                                <span class="text-right text-zinc-800 break-all">{{ $buyer['correo'] ?? ($usuario->correo ?? ($guest['correo'] ?? '—')) }}</span>
                                            </div>
                                            <div class="flex items-start justify-between gap-3 py-1">
                                                <span class="font-semibold text-zinc-900">Documento</span>
                                                <span class="text-right text-zinc-800">{{ trim((string) (($buyer['tipo_documento'] ?? ($perfil->tipo_documento ?? ($guest['tipo_documento'] ?? ''))) . ' ' . ($buyer['numero_documento'] ?? ($perfil->numero_documento ?? ($guest['numero_documento'] ?? ''))))) ?: '—' }}</span>
                                            </div>
                                            <div class="flex items-start justify-between gap-3 py-1">
                                                <span class="font-semibold text-zinc-900">Teléfono</span>
                                                <span class="text-right text-zinc-800">{{ $buyer['telefono'] ?? ($perfil->telefono ?? ($guest['telefono'] ?? '—')) }}</span>
                                            </div>
                                            <div class="flex items-start justify-between gap-3 py-1">
                                                <span class="font-semibold text-zinc-900">Ciudad</span>
                                                <span class="text-right text-zinc-800">{{ $buyer['ciudad'] ?? ($perfil->ciudad ?? ($guest['ciudad'] ?? '—')) }}</span>
                                            </div>

                                            <div class="rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-zinc-900 shadow-sm">
                                                {{ $buyer['direccion'] ?? ($perfil->direccion ?? ($guest['direccion'] ?? '—')) }}
                                            </div>
                                            ...
                                            @if($fechaNacimiento)
                                                <div class="flex items-start justify-between gap-3 py-1">
                                                    ...
                                                    <span class="text-right text-zinc-800">{{ $fechaNacimiento }}</span>
                                                </div>
                                            @endif
                                            ...
                                            @if($genero)
                                                <div class="flex items-start justify-between gap-3 py-1">
                                                    ...
                                                    <span class="text-right text-zinc-800">{{ $genero }}</span>
                                                </div>
                                            @endif
                                        ...
                                        <div class="space-y-2 text-zinc-800">
                                            ...
                                            @else
                                                ...
                                                <p><span class="font-semibold">Tipo:</span> {{ $tipoLenteLabel }}</p>
                                                @if($lensType)
                                                    <p><span class="font-semibold">{{ $sinFormula ? 'Tratamiento' : 'Diseño' }}:</span> {{ $lensTypeSummaryLabel }}</p>
                                                @elseif($sinFormula)
                                                    <p><span class="font-semibold">Tratamiento:</span> Solo montura</p>
                                                @endif
                                                @if(!$sinFormula && $naraLevel)
                                                    <p><span class="font-semibold">NARA:</span> {{ $naraLabel }}</p>
                                                @endif
                                                @if($lensColor)
                                                    <p><span class="font-semibold">Color:</span> {{ $lensColor }}</p>
                                                @endif
                                                @if(!$sinFormula)
                                                        ...
                                                        <div class="pt-2">
                                                            ...
                                                            <div class="mt-2 grid gap-2 sm:grid-cols-2 text-[12px] text-zinc-800">
                                                                <div class="rounded-2xl border border-zinc-200 bg-white px-3 py-2">
                                                                    ...
                                                                    <p>Esfera: {{ (string) ($od['esfera'] ?? '') !== '' ? $od['esfera'] : '—' }}</p>
                                                                    <p>Cilindro: {{ (string) ($od['cilindro'] ?? '') !== '' ? $od['cilindro'] : '—' }}</p>
                                                                    <p>Eje: {{ (string) ($od['eje'] ?? '') !== '' ? $od['eje'] : '—' }}</p>
                                                                    <p>Adición: {{ (string) ($od['adicion'] ?? '') !== '' ? $od['adicion'] : '—' }}</p>
                                                                </div>
                                                                <div class="rounded-2xl border border-zinc-200 bg-white px-3 py-2">
                                                                    <p class="font-semibold text-zinc-900">Ojo izquierdo (OI)</p>
                                                                    <p>Esfera: {{ (string) ($oi['esfera'] ?? '') !== '' ? $oi['esfera'] : '—' }}</p>
                                                                    <p>Cilindro: {{ (string) ($oi['cilindro'] ?? '') !== '' ? $oi['cilindro'] : '—' }}</p>
                                                                    <p>Eje: {{ (string) ($oi['eje'] ?? '') !== '' ? $oi['eje'] : '—' }}</p>
                                                                    <p>Adición: {{ (string) ($oi['adicion'] ?? '') !== '' ? $oi['adicion'] : '—' }}</p>
                                                                </div>
                                                            </div>

                                                            <div class="mt-2 rounded-2xl border border-zinc-200 bg-white px-3 py-2 text-[12px] text-zinc-800">
                                                                <p><span class="font-semibold">DNP:</span> {{ (string) ($manual['distancia_pupilar'] ?? '') !== '' ? $manual['distancia_pupilar'] : '—' }}</p>
                                                                <p><span class="font-semibold">Año de nacimiento:</span> {{ (string) ($manual['ano_nacimiento'] ?? '') !== '' ? $manual['ano_nacimiento'] : '—' }}</p>
                                                            </div>
                                                        </div>
                                                    @elseif($rxSphereMax !== null || $rxCylMax !== null)
                                                        <p><span class="font-semibold">RX máx:</span> Esfera {{ $rxSphereMax ?? '—' }}, Cilindro {{ $rxCylMax ?? '—' }}</p>
                                                    @endif
                                                @endif

                                                @if($precioMontura !== null || $precioLentes !== null || $precioTotal !== null)
                                                    <div class="space-y-0.5 pt-2">
                                                        @if($precioMontura !== null)
                                                            <p><span class="font-semibold">Montura:</span> {{ number_format((float) $precioMontura, 0, ',', '.') }} {{ $pago->moneda }}</p>
                                                        @endif
                                                        @if($precioLentes !== null)
                                                            <p><span class="font-semibold">Lentes:</span> {{ number_format((float) $precioLentes, 0, ',', '.') }} {{ $pago->moneda }}</p>
                                                        @endif
                                                        @if($precioTotal !== null)
                                                            <p><span class="font-semibold">Total:</span> {{ number_format((float) $precioTotal, 0, ',', '.') }} {{ $pago->moneda }}</p>
                                                        @endif
                                                    </div>
                                                @endif
                                            @endif
