...
<head>
    ...
    <link rel="icon" href="{{ $seoFavicon }}" sizes="any">
    ...
    <style>
        ...
        .hero-panel-image.is-exit {
            transform: translateX(100%);
            z-index: 1;
        }

        /* Overlay de texto del hero */
        .hero-content-overlay {
            ...
            text-align: center;
            padding: 2rem 1.5rem;
            background: linear-gradient(180deg, rgba(0,0,0,0.15) 0%, rgba(0,0,0,0.50) 55%, rgba(0,0,0,0.75) 100%);
        }
        ...
        .hero-btn-primary:hover { background: #c49558; transform: translateY(-1px); }
        ...
        .hero-btn-secondary:hover { border-color: #fff; transform: translateY(-1px); }
        ...
        .hero-stars { color: #d4a96a; letter-spacing: 0.05em; font-size: 1rem; }
        ...
        .icon-btn:hover {
            background: rgba(75, 145, 150, 0.08);
            color: var(--na-primary-dark);
        ...
        .image-slot {
            ...
            align-items: center;
            justify-content: center;
            border: 2px dashed rgba(75, 145, 150, 0.28);
            border-radius: 1.5rem;
            background:
                linear-gradient(135deg, rgba(255,255,255,0.85), rgba(244,248,249,0.95));
        ...
        .secondary-headline-accent { color: #c9a96e; }
        ...
        .secondary-cta:hover { background: #b8955a; transform: translateY(-1px); }
        ...
        .footer-brand-accent { color: #c9a96e; }
        ...
        .faq-modal-panel {
            border: 2px solid rgba(75, 145, 150, 0.78);
            box-shadow: 0 22px 56px rgba(18, 59, 64, 0.34), 0 0 0 3px rgba(75, 145, 150, 0.18);
            background:
                radial-gradient(circle at 12% 0%, rgba(75, 145, 150, 0.14), transparent 34%),
                linear-gradient(180deg, #ffffff 0%, #f9fcfc 100%);
        }

        .faq-modal-header {
            background: linear-gradient(135deg, rgba(75, 145, 150, 0.14) 0%, rgba(255, 255, 255, 0.98) 62%);
            border-bottom: 1px solid rgba(75, 145, 150, 0.24);
        }
...
<body>
    {{-- Landing 2: primera sección del nuevo home inspirada en la referencia enviada --}}
    <div id="landingGlobalLoading"
        ...
        <div class="landing-global-loading-content">
            <img src="{{ asset('images/NARA PERRO.gif') }}" alt="Cargando" class="landing-global-loading-gif" loading="eager" decoding="async">
    ...
    <div class="min-h-screen bg-white text-zinc-900">
        ...
        <main>
            {{-- Hero principal: título, mensaje de envíos y espacios de imágenes --}}
            <section class="hero-panel">
                ...
                @endphp
                {{-- Frame: solo contiene las imágenes del slider --}}
                ...
                {{-- Overlay: hermano del frame, directo dentro de .hero-panel (position: relative) --}}
            ...
            <script>
                (() => {
                    ...
                    const seconds = Number(@json((int) ($landingHeroCarousel['seconds_per_slide'] ?? 5)));
            ...
            {{-- Franja de beneficios como en la referencia --}}
            <section id="beneficios" class="benefit-strip border-b border-[#e7d9e0] py-2">
                @php
                ...
                <div class="benefit-marquee w-full px-0">
                    <div class="benefit-track text-sm font-semibold text-zinc-800 sm:text-[1.05rem]" aria-label="Beneficios incluidos">
                        @foreach(array_merge(($benefitItems ?? []), ($benefitItems ?? [])) as $benefit)
            ...
            {{-- Vitrina de productos destacada --}}
            <section id="catalogo" class="py-10 sm:py-14">
                <div class="mx-auto max-w-7xl px-4">
                    <div class="flex items-end justify-between mb-8">
                        <div>
                            <span class="section-eyebrow" style="text-align:left;">SELECCIÓN PREMIUM</span>
                            <h2 class="text-left text-[2.4rem] font-extrabold leading-[1.05] tracking-tight text-black sm:text-[3rem]">
                                Nuestras monturas<br>
                                <span style="color:#c9a96e;">favoritas</span>
                            </h2>
                        </div>
                        <a href="{{ url('/gafas') }}" class="catalog-header-btn hidden md:inline-flex">Ver colección completa</a>
                    </div>
                    ...
                    <div class="mt-8 hidden overflow-hidden md:block" data-catalog-carousel-desktop>
                        <div class="flex" data-catalog-track-desktop>
                            @if($catalogDesktopSlides->count() > 0)
                                @foreach($catalogDesktopSlides as $slide)
                                    <div class="catalog-slide w-full shrink-0">
                                        <div class="grid grid-cols-2 gap-3 sm:gap-5 xl:grid-cols-4">
                                            @foreach($slide as $product)
                                                ...
                                                @endphp
                                                <a href="{{ route('gafas.show', ['producto' => $product->slug]) }}" class="product-card product-reveal block" title="Ver {{ $product->nombre }}">
                                                    @if($img !== '')
                                                        <div class="mx-auto aspect-[5/2.6] w-full max-w-[10.5rem] overflow-hidden rounded-[1rem] sm:max-w-[13rem] xl:max-w-[16rem]">
                                                            <img src="{{ $img }}" alt="{{ e($product->nombre) }}" class="h-full w-full object-contain" loading="lazy" draggable="false">
                                                        </div>
                                                    @else
                                                        <div class="image-slot product-image-slot mx-auto aspect-[5/2.6] w-full max-w-[10.5rem] rounded-[1rem] sm:max-w-[13rem] xl:max-w-[16rem]">
                                                            <span>Sin imagen</span>
                                                        </div>
                                                    @endif
                                                    <h3 class="mt-2 line-clamp-1 text-[0.95rem] font-extrabold text-black sm:mt-3 sm:text-[1.05rem]">{{ $product->nombre }}</h3>
                                                    <p class="text-[0.95rem] text-zinc-700 sm:text-[1.05rem]">{{ $price !== null ? '$ ' . number_format((float) $price, 0, ',', '.') : '—' }}</p>
                                                </a>
                                            @endforeach
                                        </div>
                                    </div>
                                ...
                                <div class="catalog-slide w-full shrink-0">
                                    <div class="grid grid-cols-2 gap-4 sm:gap-5 xl:grid-cols-4">
                                        @foreach($placeholders as $ph)
                                            <a href="{{ url('/gafas') }}" class="product-card product-reveal block" title="Ver catálogo completo">
                                                <div class="image-slot product-image-slot" style="position:relative;">
                                                    <span class="product-badge {{ $ph['dark'] ? 'is-dark' : '' }}">{{ $ph['badge'] }}</span>
                                                    <span>Espacio imagen</span>
                                                </div>
                                                <span class="product-brand">{{ $ph['brand'] }}</span>
                                                <h3>{{ $ph['name'] }}</h3>
                                                <p>{{ $ph['price'] }}</p>
                                            </a>
                                        @endforeach
                    ...
                    <div class="mt-8 overflow-hidden md:hidden" data-catalog-carousel-mobile>
                        <div class="flex" data-catalog-track-mobile>
                            @if($catalogMobileSlides->count() > 0)
                                @foreach($catalogMobileSlides as $slide)
                                    <div class="catalog-slide w-full shrink-0">
                                        <div class="grid grid-cols-2 gap-3">
                                            @foreach($slide as $product)
                                                ...
                                                <a href="{{ route('gafas.show', ['producto' => $product->slug]) }}" class="product-card product-reveal block" title="Ver {{ $product->nombre }}">
                                                    @if($img !== '')
                                                        <div class="mx-auto aspect-[5/2.6] w-full max-w-[10.5rem] overflow-hidden rounded-[1rem]">
                                                            <img src="{{ $img }}" alt="{{ e($product->nombre) }}" class="h-full w-full object-contain" loading="lazy" draggable="false">
                                                        </div>
                                                    @else
                                                        <div class="image-slot product-image-slot mx-auto aspect-[5/2.6] w-full max-w-[10.5rem] rounded-[1rem]">
                                                            <span>Sin imagen</span>
                                                        </div>
                                                    @endif
                                                    <h3 class="mt-2 line-clamp-1 text-[0.95rem] font-extrabold text-black">{{ $product->nombre }}</h3>
                                                    <p class="text-[0.95rem] text-zinc-700">{{ $price !== null ? '$ ' . number_format((float) $price, 0, ',', '.') : '—' }}</p>
                                                </a>
                                            @endforeach
                                        </div>
                                    </div>
                            ...
                            @else
                                <div class="catalog-slide w-full shrink-0">
                                    <div class="grid grid-cols-2 gap-3">
                                        @foreach(array_slice($placeholders ?? [['brand'=>'RAY-BAN','name'=>'Classic Aviator','price'=>'$ 289.000','badge'=>'NUEVO','dark'=>false],['brand'=>'HAWKERS','name'=>'Urban Square','price'=>'$ 195.000','badge'=>'EXCLUSIVO','dark'=>true]], 0, 2) as $ph)
                                            <a href="{{ url('/gafas') }}" class="product-card product-reveal block" title="Ver catálogo completo">
                                                <div class="image-slot product-image-slot" style="position:relative;">
                                                    <span class="product-badge {{ $ph['dark'] ? 'is-dark' : '' }}">{{ $ph['badge'] }}</span>
                                                    <span>Espacio imagen</span>
                    ...
                    <div class="mt-5 hidden items-center justify-center gap-3 md:flex" data-catalog-dots-desktop>
                        @for($i = 0; $i < $desktopDotCount; $i++)
                            <button type="button" class="slider-dot {{ $i === 0 ? 'is-active' : '' }}" data-catalog-dot-desktop data-index="{{ $i }}" aria-label="Ir a productos {{ $i + 1 }}"></button>
                        @endfor
                    </div>

                    <div class="mt-5 flex items-center justify-center gap-3 md:hidden" data-catalog-dots-mobile>
                        @for($i = 0; $i < $mobileDotCount; $i++)
                            <button type="button" class="slider-dot {{ $i === 0 ? 'is-active' : '' }}" data-catalog-dot-mobile data-index="{{ $i }}" aria-label="Ir a productos {{ $i + 1 }}"></button>
                        @endfor
                    </div>

                    <div class="mt-2 text-center">
                        <a href="{{ url('/gafas') }}" class="showcase-link text-[1.8rem] font-extrabold uppercase tracking-tight">VER MÁS</a>
                    </div>
                </div>
            </section>
                    ...
                    <div class="mt-8 grid gap-4 md:grid-cols-3">
                        @foreach($categoryCards as $category)
                            <a
                                ...
                                data-reveal-side="{{ $loop->index % 2 === 0 ? 'left' : 'right' }}"
                                title="Ver categoría {{ $category['label'] }}"
                            >
                                <div class="category-photo-slot{{ !empty($category['image']) ? ' has-cover-image' : '' }}" style="{{ $category['style'] }}">
                                    @if(!empty($category['image']))
                                        <img src="{{ $category['image'] }}" alt="{{ $category['label'] }}" class="category-photo-image" loading="lazy" draggable="false" style="object-position: {{ $category['image_position'] ?? 'center' }};">
                                    @else
                                        <span class="category-placeholder-tag">{{ $category['placeholder'] }}</span>
                                    @endif
                                ...
                                <div class="category-label">
                                    @if(!empty($category['tagline']))
                                        <span class="category-label-tagline">{{ $category['tagline'] }}</span>
                                    @endif
                                    <span class="category-label-name">{{ $category['label'] }}</span>
                                </div>
                            </a>
                        @endforeach
                    </div>
            ...
            {{-- Bloque extra inspirado en la referencia de tratamiento polarizado --}}
            <section id="polarizado" class="polarized-feature-shell">
                <div class="polarized-feature-card">
                    @php
                        $secondaryFallback = asset('images/Promo2.png');
                        $secondaryImages = [];
                    ...
                    {{-- Frame: solo imágenes del slider --}}
                    ...
                    <div class="secondary-text-overlay">
                        ...
                        <a href="{{ url('/gafas') }}" class="secondary-cta">
                            Conocer más &nbsp;→
                        </a>
                    </div>
                </div>
            ...
            <script>
                (() => {
                    ...
                    const images = @json($secondaryImages ?? []);
                    if (!imageA || !imageB || !Array.isArray(images) || images.length < 2) return;
                    ...
                    let frontIsA = true;
                    const total = images.length;
                    const seconds = Number(@json((int) ($landingSecondaryCarousel['seconds_per_slide'] ?? 5)));
                    const delayMs = (Number.isFinite(seconds) ? Math.max(2, Math.min(30, seconds)) : 5) * 1000;
                    const slideDurationMs = 900;
            ...
            {{-- Sección de ubicación replicando la referencia --}}
            <section id="ubicacion" class="py-14 sm:py-20" style="background:#111827;">
                <div class="mx-auto max-w-7xl px-4">
                    <span class="section-eyebrow" style="color:#c9a96e;">DÓNDE ESTAMOS</span>
                    <h2 class="mt-1 text-center text-3xl font-extrabold tracking-tight text-white sm:text-[3.25rem]">
                        {{ (string) ($landingLocation['title'] ?? 'Visítanos en persona') }}
                    </h2>
                    ...
                    <div class="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach($locations as $location)
                            ...
                            <a href="{{ $href }}" target="_blank" rel="noopener"
                               class="location-city-card location-reveal {{ $isOddLastCard ? 'sm:col-span-2 lg:col-span-1' : '' }}"
                               title="Abrir ubicación de {{ $cityName }} en Google Maps">

                                {{-- Foto de la sede --}}
                                <div class="image-slot storefront-slot">
                                    <img src="{{ $imageTop }}" alt="{{ $venueName }}" loading="lazy" draggable="false">
                                </div>
                                ...
                                <div class="location-card-body">
                                    ...
                                    <h3 class="location-city-name">{{ $cityName }}</h3>
            ...
            {{-- Último apartado: reseñas + formulario --}}
                ...
                <div class="mx-auto max-w-7xl px-4">
                    ...
                    <div class="flex flex-col items-center justify-center gap-3 text-center">
                        ...
                        <div class="inline-flex items-center gap-2 mt-1">
                            <span class="text-lg text-[#c9a96e] leading-none">{{ str_repeat('★', max(5, $reviewsStarsRounded)) }}</span>
                            <span class="text-sm font-semibold text-zinc-600">
                                {{ $reviewsAvgGlobal > 0 ? number_format($reviewsAvgGlobal, 1, '.', '') : '4.9' }} / 5
                                basado en
                                {{ $reviewsCountGlobal > 0 ? '+' . number_format($reviewsCountGlobal, 0, ',', '.') : '+2.100' }}
                                reseñas
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="mt-7 reviews-marquee full-bleed" data-reviews-marquee>
                        <div class="reviews-track {{ ($isReviewsAnimated ?? false) ? 'is-animated' : 'is-static' }}" data-reviews-track>
                        @php
                            $reviewGroups = ($isReviewsAnimated ?? false) ? [0, 1] : [0];
                        @endphp
                        @foreach($reviewGroups as $reviewGroupIndex)
                        <div class="reviews-group" data-reviews-group @if(($isReviewsAnimated ?? false) && $reviewGroupIndex === 1) aria-hidden="true" @endif>
                        @foreach((($isReviewsAnimated ?? false) ? $reviewsLoopItems : $reviewsItems) as $review)
                            @php
                            ...
                            <article
                                ...
                                data-review-card
                                data-review-name="{{ $reviewName }}"
                                data-review-handle="{{ $handle }}"
                                data-review-stars="{{ $reviewStars }}"
                                data-review-comment="{{ $reviewComment }}"
                                data-review-photo="{{ $reviewPhoto }}"
                                data-review-initials="{{ $initials }}"
                                tabindex="0"
                                role="button"
                                aria-label="Ver detalle de reseña de {{ $reviewName }}"
                            >
                                {{-- Estrellas arriba --}}
                                <div class="flex items-center justify-between">
                                    <div class="text-base text-[#c9a96e]">{{ str_repeat('★', $reviewStars) }}<span class="text-zinc-200">{{ str_repeat('★', 5 - $reviewStars) }}</span></div>
                                    @if($canDeleteReviews && !is_array($review) && isset($review->id))
                                        <form method="POST" action="{{ route('resenas.destroy', $review) }}" onsubmit="return confirm('¿Eliminar esta reseña?');" data-review-delete-form>
                                            @csrf @method('DELETE')
                                ...
                                <div class="flex items-center gap-3 pt-2 border-t border-zinc-100">
                                    ...
                                    <div>
                                        <p class="text-sm font-extrabold text-zinc-800">{{ $reviewName }}</p>
                                        <p class="text-xs text-zinc-400">{{ $handle }}</p>
                                    </div>
                                </div>
                            </article>
                        @endforeach
                    ...
                    <div class="mt-8 flex justify-center">
                        <a href="{{ route('resenas.index') }}" class="catalog-header-btn">
                            Ver todas las reseñas
                        </a>
                    </div>

                    <form action="{{ route('resenas.store') }}" method="POST" enctype="multipart/form-data" class="mt-8 review-form-shell" data-review-upload-form>
                        @csrf
                        @if ($errors->has('autor_nombre'))
                            <div class="mb-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">
                                {{ $errors->first('autor_nombre') }}
                            </div>
                        @endif
                        ...
                        <div class="grid gap-4 lg:grid-cols-3">
                            <div>
                                ...
                                <input id="resena_nombre" name="autor_nombre" type="text" value="{{ old('autor_nombre') }}" placeholder="Tu nombre" class="w-full rounded-2xl border border-zinc-300 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-zinc-200">
                            </div>
                            <div>
                                ...
                                <select id="resena_estrellas" name="estrellas" class="w-full rounded-2xl border border-zinc-300 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-zinc-200">
                                    <option value="5" {{ (string) old('estrellas', '5') === '5' ? 'selected' : '' }}>5</option>
                                    <option value="4" {{ (string) old('estrellas') === '4' ? 'selected' : '' }}>4</option>
                                    <option value="3" {{ (string) old('estrellas') === '3' ? 'selected' : '' }}>3</option>
                                    <option value="2" {{ (string) old('estrellas') === '2' ? 'selected' : '' }}>2</option>
                                    <option value="1" {{ (string) old('estrellas') === '1' ? 'selected' : '' }}>1</option>
                                </select>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="resena_comentario" name="comentario" rows="4" maxlength="1000" placeholder="Cuéntanos tu experiencia" class="w-full rounded-2xl border border-zinc-300 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-zinc-200" data-comment-input data-comment-max="1000">{{ old('comentario') }}</textarea>
        ...
        <footer id="footer-info" class="footer-shell text-white">
            <div class="mx-auto max-w-7xl px-4 py-14 sm:py-16">
                <div class="grid grid-cols-2 gap-10 lg:grid-cols-4 lg:items-start">
                    ...
                    {{-- Columna 1: Marca + tagline + redes --}}
                    <div class="col-span-2 lg:col-span-1">
                        <div class="flex items-center gap-2">
                            <img src="{{ asset('images/navbar.png') }}" alt="Logo" class="h-8 w-auto object-contain">
                        ...
                        <div class="mt-5 flex items-center gap-3">
                            @forelse($landingFooter['social_links'] as $social)
                                <a href="{{ $social['href'] }}"
                                   class="social-pill {{ $social['color'] }}"
                                   onclick="return openSocialLink(event, @js($social['platform'] ?? ''), @js($social['href'] ?? '#'))"
                                   aria-label="{{ $social['platform'] ?? 'Red social' }}">{{ $social['icon'] }}</a>
                            @empty
                                <a href="#" class="social-pill" style="background:rgba(255,255,255,0.12);" onclick="return openSocialLink(event,'Instagram','#')">ig</a>
                                <a href="#" class="social-pill" style="background:rgba(255,255,255,0.12);" onclick="return openSocialLink(event,'Facebook','#')">f</a>
                                <a href="#" class="social-pill" style="background:rgba(255,255,255,0.12);" onclick="return openSocialLink(event,'TikTok','#')">♪</a>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div class="col-span-1">
                        ...
                        <a href="{{ url('/gafas-hombre') }}" class="footer-link">Hombre</a>
                        <a href="{{ url('/gafas-mujeres') }}" class="footer-link">Mujer</a>
                        <a href="{{ url('/gafas?categories[]=ninos') }}" class="footer-link">Infantil</a>
                        <a href="{{ url('/gafas?categories[]=gafas_polarizadas') }}" class="footer-link">Deportivas</a>
                        <a href="{{ url('/gafas') }}" class="footer-link">Ver todo</a>
                    </div>

                    {{-- Columna 3: Ayuda (FAQ dinámico) --}}
                    <div class="col-span-1">
                        <p class="footer-col-title">AYUDA</p>
                        @forelse($landingFooter['faq'] as $faqItem)
                            <button type="button" onclick="openFaqModal({{ $loop->index }})"
                    ...
                    {{-- Columna 4: Empresa (about_links dinámico) + contacto --}}
                    <div class="col-span-1">
                        ...
                        @forelse($landingFooter['about_links'] as $link)
                            ...
                            <a href="{{ $resolvedHref }}" class="footer-link" @if($isExternal) target="_blank" rel="noopener noreferrer" @endif>
                                {{ $link['text'] }}
                            </a>
                        @empty
                        ...
                        @if($contactEmail !== '' || $contactPhone !== '')
                            ...
                            @endif
                            @if($contactPhone !== '')
                                <a href="tel:{{ preg_replace('/[^0-9+]/', '', $contactPhone) }}" class="footer-link">{{ $contactPhone }}</a>
                            @endif
                        @endif
                    </div>
                ...
                <hr class="footer-divider mt-12 mb-6">
                <p class="text-center text-xs" style="color:rgba(255,255,255,0.25);">
                    © {{ date('Y') }} ÓpticaVision. Todos los derechos reservados.
                </p>
            </div>
        </footer>
            ...
            <div id="faqModalPanel" class="faq-modal-panel w-full translate-y-8 transform overflow-hidden rounded-t-3xl shadow-2xl transition-all duration-300 sm:max-w-2xl sm:translate-y-0 sm:rounded-3xl" style="pointer-events: none;">
                <div class="faq-modal-header sticky top-0 z-10 px-6 py-4 sm:px-8">
                    <div class="flex items-center justify-between">
                        ...
                        <button onclick="closeFaqModal()" class="flex h-8 w-8 items-center justify-center rounded-full text-zinc-500 transition hover:bg-[#4b9196]/20 hover:text-[#245c60] text-xl leading-none">&times;</button>
                    </div>
                </div>

                <div class="max-h-[65vh] overflow-y-auto divide-y divide-[#4b9196]/14 px-6 sm:px-8">
                    @forelse($landingFooter['faq'] as $faqItem)
                        ...
                        <div class="faq-accordion">
                            ...
                            >
                                <span class="text-base font-semibold text-zinc-900 leading-snug">{{ $faqItem['question'] }}</span>
                                @if($hasAnswer)
                            ...
                            @if($hasAnswer)
                                <div class="faq-body max-h-0 overflow-hidden transition-all duration-300 ease-in-out">
                                    <div class="faq-body-content pb-5 text-[0.95rem] leading-relaxed text-zinc-600">{!! nl2br(e($faqItem['answer'])) !!}</div>
                                </div>
                            @endif
                        </div>
                    @empty
                ...
                <div class="border-t border-[#4b9196]/20 bg-[#edf7f8] px-6 py-4 sm:px-8">
                    <button onclick="closeFaqModal()" class="w-full rounded-xl bg-gradient-to-r from-[#2f787d] via-[#3f888d] to-[#4b9196] px-4 py-3 text-sm font-semibold text-white shadow-[0_10px_24px_rgba(39,101,106,0.35)] transition-all hover:-translate-y-0.5 hover:from-[#286c71] hover:via-[#377f84] hover:to-[#418f94]">
                        Cerrar
                    </button>
                </div>
            </div>
