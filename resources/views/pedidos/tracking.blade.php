...
<main class="mx-auto max-w-5xl px-4 py-6 sm:py-10">
    <section id="seguimiento-pedido" class="overflow-hidden rounded-3xl border border-[#bde5e8] bg-gradient-to-br from-[#eefafa] via-white to-[#f7fdff] p-4 shadow-sm sm:p-6">
        <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
            ...
            <a href="{{ route('landing') }}" class="rounded-2xl px-3 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-100">Volver a inicio</a>
        </div>

        <form method="GET" action="{{ route('pedido.tracking') }}" class="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
            <div>
                <label for="cedula" class="text-xs font-semibold text-zinc-700">Cédula</label>
                <input id="cedula" name="cedula" value="{{ $trackingInputCedula ?? '' }}" placeholder="Número de cédula" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-[#1f9096]" />
            </div>
        ...
        @if(($trackingResults ?? collect())->isNotEmpty())
            <div class="mt-6">
                <div class="mb-3 flex items-center justify-between gap-3">
                    ...
                    <span class="inline-flex items-center rounded-full bg-white px-3 py-1 text-xs font-semibold text-zinc-700 ring-1 ring-zinc-200">
                        {{ $trackingResults->count() }} resultados
                    </span>
                </div>

                <div class="grid gap-3">
                    @foreach($trackingResults as $pedido)
                        <article class="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    <p class="text-base font-bold text-zinc-900">{{ $pedido['cliente'] }}</p>
                                    <p class="mt-1 text-sm text-zinc-600">Referencia: {{ $pedido['referencia'] }}</p>
                                </div>
                                <div class="flex flex-wrap gap-2">
                                    <span class="inline-flex items-center rounded-full bg-sky-50 px-3 py-1 text-xs font-semibold text-sky-700">{{ $pedido['estado_pago'] }}</span>
                                    <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold {{ $pedido['esta_enviado'] ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700' }}">{{ $pedido['estado_envio'] }}</span>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 grid gap-2 text-sm text-zinc-700 sm:grid-cols-3">
                                <div class="rounded-2xl bg-zinc-50 px-3 py-2">
                                    ...
                                    {{ number_format((float) $pedido['monto'], 0, ',', '.') }} {{ $pedido['moneda'] }}
                                </div>
                                <div class="rounded-2xl bg-zinc-50 px-3 py-2">
                                ...
                                <div class="rounded-2xl bg-zinc-50 px-3 py-2">
                                    ...
                                    {{ $pedido['fecha_envio'] ?: 'Aún sin marcar envío' }}
                                </div>
                                <div class="rounded-2xl bg-zinc-50 px-3 py-2">
                                    <span class="font-semibold text-zinc-900">Comprobante:</span>
                                    @if(!empty($pedido['shipping_file']))
                                        <a href="{{ $pedido['shipping_file'] }}" class="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1 text-xs font-semibold text-zinc-700 border border-zinc-200" target="_blank" rel="noopener">Descargar PDF / Foto</a>
                                    @else
                            ...
                            @if(!empty($pedido['shipping_file']) && $pedido['esta_enviado'])
                                <div class="mt-4">
                                    <div class="rounded-2xl border border-zinc-200 bg-white p-3">
                                        <div class="flex items-center justify-between gap-3">
                                            <div class="text-sm text-zinc-800">Comprobante subido: <strong>{{ $pedido['shipping_file_name'] ?? 'archivo' }}</strong></div>
                                            <div>
                                                <a href="{{ $pedido['shipping_file'] }}" download class="inline-flex items-center gap-2 rounded-full bg-[#1f9096] px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-[#187b80]">Descargar PDF aquí</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
