...
<main class="mx-auto max-w-7xl px-4 py-8">
    <div class="mb-6 text-center">
        ...
        <h1 class="mt-1 text-3xl font-extrabold tracking-tight text-zinc-900 sm:text-4xl">{{ $pageTitle }}</h1>
        <p class="mt-2 text-base text-zinc-500">{{ $pageSubtitle }}</p>
    </div>
    ...
    <div class="grid gap-6">
        <aside id="mobile-filters-panel" class="hidden fixed inset-0 z-[90] items-start justify-center overflow-hidden bg-black/35 p-2.5 sm:p-4 md:items-center" style="padding-top: max(0.625rem, env(safe-area-inset-top)); padding-right: max(0.625rem, env(safe-area-inset-right)); padding-bottom: max(0.625rem, env(safe-area-inset-bottom)); padding-left: max(0.625rem, env(safe-area-inset-left));">
            <div data-filters-card class="relative flex w-full max-w-[22rem] flex-col overflow-hidden rounded-[1.75rem] border border-zinc-200 bg-white p-3.5 sm:max-w-sm sm:rounded-3xl sm:p-5">
                ...
                <form id="gafasFiltersForm" method="GET" action="{{ url()->current() }}" class="mt-3 pr-1 sm:mt-4 md:pr-2" data-auto-submit-filters>
                ...
                <div class="mt-4 border-t border-zinc-200 pt-4 sm:mt-6 sm:pt-6">
                    ...
                    <div class="mt-4 border-t border-zinc-200 pt-4 sm:mt-5 sm:pt-5">
                        ...
                        <div class="mt-2.5 grid gap-2 sm:mt-3">
                            <label class="block cursor-pointer">
                                <input type="checkbox" name="progresivos" value="1" class="peer sr-only" {{ $progresivosFilterActive ? 'checked' : '' }}>
                                <span class="flex items-center gap-2.5 rounded-[1.15rem] border border-zinc-200 bg-white px-3 py-2.5 text-[13px] font-semibold text-zinc-900 transition hover:bg-zinc-50 sm:gap-3 sm:rounded-2xl sm:px-4 sm:py-3 sm:text-sm peer-checked:border-zinc-950 peer-checked:bg-zinc-950 peer-checked:text-white peer-checked:[&_.js-filter-check]:border-zinc-900 peer-checked:[&_.js-filter-check]:text-zinc-900">
                                    <span class="js-filter-check inline-flex h-4 w-4 items-center justify-center rounded border border-zinc-300 bg-white text-transparent text-[11px] font-bold leading-none">
                                        ✔
                                    </span>
                    ...
                    <div class="mt-4 grid grid-cols-2 gap-2 sm:mt-5">
                        ...
                        <a href="{{ route('gafas.index') }}" class="inline-flex items-center justify-center rounded-[1.15rem] border border-zinc-200 bg-white px-3 py-2.5 text-[13px] font-semibold text-zinc-900 hover:bg-zinc-50 sm:rounded-2xl sm:px-4 sm:py-3 sm:text-sm">
                            Limpiar
                        </a>
                    </div>
                </form>
                        ...
                        <div class="group relative mx-auto w-full max-w-[290px]">
                            ...
                            <a href="{{ route('gafas.show', ['producto' => $producto->slug]) }}" class="block" data-gafa-link>
                                <div class="pt-2 text-center">
                                    ...
                                    <div class="mt-2 flex {{ $swatchWrapClass }} items-center justify-center {{ $swatchGapClass }} overflow-visible py-0.5">
                                        @foreach($colorGroups as $i => $group)
                                            @php
                                            ...
                                            <button
                                                type="button"
                                                class="gafa-color-swatch inline-block shrink-0 {{ $swatchSizeClass }} rounded-full border-2 border-white shadow-sm transition overflow-hidden p-0 data-[active=true]:scale-125 data-[active=true]:shadow-md"
                                                style="{{ !$isMulti ? 'background-color: ' . $firstHex . ';' : '' }}"
                                                data-color-hex="{{ $firstHex }}"
                                                data-color-image="{{ $groupImage }}"
                                                data-color-name="{{ $groupName }}"
                                                data-active="{{ $i === 0 ? 'true' : 'false' }}"
                                                aria-label="Color {{ $groupName }}"
                                                aria-pressed="{{ $i === 0 ? 'true' : 'false' }}"
                                            >@if($isMulti)<svg width="100%" height="100%" viewBox="0 0 {{ $sz }} {{ $sz }}" style="display:block;">{!! $paths !!}</svg>@endif</button>
                                        @endforeach
                                    </div>
                                </div>
                            </a>
...
<section class="py-10 sm:py-14 bg-white">
    <div class="mx-auto max-w-7xl px-4">
        ...
        @if($sugerenciasProductos->isNotEmpty())
            <div class="grid grid-cols-2 gap-3 sm:gap-5 xl:grid-cols-4">
                @foreach($sugerenciasProductos as $sug)
                    ...
                    @endphp
                    <a href="{{ route('gafas.show', ['producto' => $sug->slug]) }}" class="block group" title="Ver {{ $sug->nombre }}">
                        @if($sugImg !== '')
                            <div class="mx-auto aspect-[5/2.6] w-full overflow-hidden rounded-[1rem]">
                                <img src="{{ $sugImg }}" alt="{{ e($sug->nombre) }}" class="h-full w-full object-contain transition-transform duration-300 group-hover:scale-105" loading="lazy" draggable="false">
                            </div>
                        @else
        ...
        @else
            <div class="grid grid-cols-2 gap-3 sm:gap-5 xl:grid-cols-4">
                @foreach([['Montura Clásica Premium','$ 289.000'],['Urban Square Edition','$ 195.000'],['Sport Half Frame Pro','$ 320.000'],['Minimal Round Bold','$ 175.000']] as [$phName, $phPrice])
                    <a href="{{ url('/gafas') }}" class="block">
                        <div class="mx-auto aspect-[5/2.6] w-full rounded-[1rem] bg-zinc-100 flex items-center justify-center">
                            <span class="text-xs text-zinc-400">Espacio imagen</span>
                        </div>
                        <h3 class="mt-2 text-[0.95rem] font-extrabold text-black sm:mt-3 sm:text-[1.05rem]">{{ $phName }}</h3>
                        <p class="text-[0.95rem] text-zinc-700 sm:text-[1.05rem]">{{ $phPrice }}</p>
                    </a>
                @endforeach
            </div>
        @endif
