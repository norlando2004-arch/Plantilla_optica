...
<head>
    ...
    <title>{{ $producto->nombre }} — Optica S.A.S</title>
...
@endphp
@if(is_array($extraGallery))
    @foreach($extraGallery as $g)
        @php
        ...
        <section class="lg:col-span-7">
            <div class="px-2 pt-2">
                <div class="mx-auto w-full max-w-[42rem]">
                    <div id="gafaZoomArea" class="relative aspect-[4/3] w-full select-none overflow-hidden sm:aspect-[16/9]">
                        @if($primaryImg)
                            <img
                                ...
                                data-base-scale="{{ number_format($zoom, 2, '.', '') }}"
                                data-default-origin="{{ (int) round($posXEff) }}% {{ (int) round($posYEff) }}%"
                                class="absolute inset-0 h-full w-full object-contain"
                                loading="eager"
                                fetchpriority="high"
                                decoding="async"
                                style="object-position: {{ (int) round($posXEff) }}% {{ (int) round($posYEff) }}%; transform-origin: {{ (int) round($posXEff) }}% {{ (int) round($posYEff) }}%; transform: scale({{ number_format($zoom, 2, '.', '') }}); transition: transform 120ms ease-out, transform-origin 120ms ease-out;"
                            />
                            <button
                                type="button"
                                id="gafaPrevImage"
                    ...
                    <div id="gafaThumbsWrap" class="hidden" hidden aria-hidden="true">
                        ...
                        @foreach($gallery as $g)
                            <button
                                type="button"
                                class="group w-16 shrink-0 overflow-hidden rounded-2xl border border-zinc-200/70 bg-white shadow-sm transition focus:outline-none focus:ring-2 focus:ring-[#3B7C82] sm:w-auto"
                                data-gafa-thumb="{{ $g }}"
                                aria-label="Ver imagen"
                            >
                                <div class="relative aspect-square w-full bg-gradient-to-br from-zinc-50 to-white">
                                    <img src="{{ $g }}" alt="" loading="lazy" class="absolute inset-0 h-full w-full object-cover transition duration-200 group-hover:scale-[1.10] group-hover:saturate-110" />
                                </div>
                            </button>
                        @endforeach
                        </div>
                    ...
                    <div id="gafaMobileColorSection" class="mt-4 lg:hidden">
                        <p class="text-sm text-zinc-800"><span class="font-semibold">Color:</span> <span id="gafaColorLabelValueMobile">{{ $colorLabel ?? ($meta['color'] ?? ($meta['color_nombre'] ?? ($producto->color ?? ''))) }}</span></p>
                ...
                <div class="mt-6 space-y-5">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
                            ...
                            <div class="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3">
                                ...
                                <p class="mt-1 text-[11px] font-semibold uppercase tracking-wide text-zinc-400">{{ $card['label'] }}</p>
                                <p class="mt-0.5 text-sm font-semibold text-zinc-900">{{ $card['value'] }}</p>
                            </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    {{-- Bloque: medidas --}}
                    @if($vAt !== null || $vAl !== null || $vAlt !== null || $vP !== null || $vLp !== null)
                    <div>
                        ...
                        <div class="grid grid-cols-3 gap-2 sm:grid-cols-5">
                            ...
                            @endphp
                            @foreach($medidaCards as $m)
                            <div class="rounded-2xl border border-zinc-200 bg-white px-3 py-3 text-center">
                                <p class="text-base font-black text-zinc-900">{{ $m['value'] }}</p>
                                <p class="mt-1 text-[10px] font-semibold uppercase tracking-wide text-zinc-400">{{ $m['label'] }}</p>
                            </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3">
                        ...
                        <p class="mt-1 text-sm text-zinc-700 leading-relaxed">{{ $producto->descripcion }}</p>
                    </div>
                    @endif
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
                            <div class="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3">
                                ...
                                <p class="mt-0.5 text-sm font-bold {{ $compatProgresivos === 'Sí' ? 'text-emerald-600' : 'text-zinc-900' }}">{{ $compatProgresivos }}</p>
                            </div>
                            <div class="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3">
                                <p class="text-[11px] font-semibold uppercase tracking-wide text-zinc-400">Tipo de fórmula</p>
                                <p class="mt-0.5 text-sm font-bold text-zinc-900">{{ $compatTipoFormula }}</p>
                            </div>
                            @if(array_key_exists('formula_permitida', $meta))
                            <div class="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3">
                                <p class="text-[11px] font-semibold uppercase tracking-wide text-zinc-400">Fórmula médica</p>
                                <p class="mt-0.5 text-sm font-bold {{ $formulaPermitida ? 'text-emerald-600' : 'text-zinc-900' }}">{{ $formulaPermitida ? 'Sí' : 'No' }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
...
@endphp
            ...
            <div class="sticky top-24 py-4">
                ...
                {{-- Nombre --}}
                <h2 class="text-2xl font-bold leading-tight tracking-tight text-zinc-900">{{ $producto->nombre }}</h2>

                {{-- Precio --}}
                <div class="mt-2">
                    @if($producto->precio_oferta)
                        <div class="flex flex-wrap items-end gap-x-3 gap-y-1">
                            <p class="text-[3rem] font-bold leading-none text-zinc-900">$ {{ number_format((float) $producto->precio_oferta, 0, ',', '.') }}</p>
                            @if($producto->precio !== null)
                                <p class="pb-1 text-lg font-semibold text-zinc-400 line-through">$ {{ number_format((float) $producto->precio, 0, ',', '.') }}</p>
                            @endif
                        </div>
                    @else
                        <p class="text-[3rem] font-bold leading-none text-zinc-900">{{ $producto->precio !== null ? '$ ' . number_format((float) $producto->precio, 0, ',', '.') : '—' }}</p>
                    @endif
                </div>
                ...
                <div class="mt-3">
                    <p class="text-sm text-zinc-800"><span class="font-semibold">Color:</span> <span id="gafaColorLabelValue">{{ $colorLabel }}</span></p>
                    <p class="mt-1 text-sm text-zinc-800"><span class="font-semibold">Stock:</span> <span id="gafaColorStockValue" class="font-semibold {{ $stockLabelClass }}">{{ $stockLabel }}</span></p>
                    <div class="mt-2 flex flex-wrap gap-2" data-gafa-color-source="sidebar">
                        @foreach($detailGroups as $i => $group)
                            @php
                            ...
                            <button
                                type="button"
                                class="inline-block h-6 w-6 overflow-hidden rounded-full border-2 border-white shadow-[0_0_0_1.5px_rgba(0,0,0,0.18)] transition data-[active=true]:scale-110 data-[active=true]:shadow-[0_0_0_2px_rgba(24,24,27,0.85)] {{ $isSoldOut ? 'opacity-45' : '' }}"
                                style="{{ !$isMulti ? 'background-color: ' . $firstHex . ';' : '' }}"
                                data-gafa-color-swatch
                                data-color-name="{{ $groupName }}"
                                data-color-image="{{ $groupImage }}"
                                data-color-images='@json($groupImages)'
                                data-color-stock="{{ $groupStock !== null ? (int) $groupStock : '' }}"
                                data-active="{{ $isCurrent ? 'true' : 'false' }}"
                                aria-label="Color {{ $groupName }}"
                                aria-pressed="{{ $isCurrent ? 'true' : 'false' }}"
                            >@if($isMulti)<svg width="100%" height="100%" viewBox="0 0 {{ $sz }} {{ $sz }}" style="display:block;">{!! $svgPaths !!}</svg>@endif</button>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="flex items-center gap-3">
                        @unless($hideLoveCta)
                            <div class="min-w-0 flex-1">
                                <button
                                    type="button"
                                    @if($formulaPermitida) data-open-buy-panel @else data-direct-polarizada-buy @endif
                                    class="inline-flex w-full items-center justify-center rounded-2xl px-6 py-3.5 text-base font-black transition focus:outline-none focus:ring-2 bg-[#c9a96e] text-white hover:bg-[#b8935a] focus:ring-[#c9a96e]/60 {{ $agotado ? 'cursor-not-allowed opacity-50' : '' }}"
                                    {{ $agotado ? 'disabled' : '' }}
                                >
                                    {{ $agotado ? 'Color agotado' : 'Adquiere aquí' }}
                                </button>
                            </div>
                        @endunless
                        <img src="{{ asset('images/enamorado.png') }}" alt="" loading="lazy" class="h-20 w-auto shrink-0 object-contain" />
                    </div>
                                ...
                                <div class="gafa-safari-soft-surface absolute left-3 right-16 top-3 z-10 flex items-center gap-2 overflow-x-auto rounded-2xl bg-black/35 px-2 py-2 backdrop-blur-sm">
                                    @foreach($cameraGroups as $i => $group)
                                        ...
                                        <button
                                            ...
                                            style="{{ !$camIsMulti ? 'background-color: ' . $camFirstHex . ';' : '' }}"
                                            data-cam-color-swatch
                                            data-cam-color-name="{{ $camGroupName }}"
                                            data-cam-color-image="{{ $camGroupImg }}"
                                            data-active="{{ $camActive ? 'true' : 'false' }}"
                                            aria-label="Color cámara {{ $camGroupName }}"
                                            aria-pressed="{{ $camActive ? 'true' : 'false' }}"
                                        >@if($camIsMulti)<svg width="100%" height="100%" viewBox="0 0 {{ $camSz }} {{ $camSz }}" style="display:block;">{!! $camPaths !!}</svg>@endif</button>
                                    @endforeach
                                </div>
    ...
    @if(!$formulaPermitida)
    {{-- Formulario oculto para compra directa sin fórmula (polarizadas y deportivas/descanso con formula_permitida=No) --}}
    <form id="directPolarizadaBuyForm" action="{{ route('checkout.gafa', ['producto' => $producto->slug]) }}" method="GET" class="hidden">
        <input type="hidden" name="no_prescription" value="1">
        <input type="hidden" name="frame_color" value="{{ e((string)($colorLabel ?? ($producto->color ?? ''))) }}" data-direct-frame-color>
    </form>
    @endif
        ...
        <section class="mt-16">
            ...
            <div class="related-minimal-wrap relative mx-auto mt-5 w-full max-w-6xl overflow-hidden px-4 md:px-14" data-related-carousel>
                <div class="mb-4 flex items-center justify-center gap-3 md:mb-5">
                    <button type="button" data-related-prev aria-label="Anterior" class="inline-flex h-9 w-9 select-none items-center justify-center rounded-full bg-zinc-900 text-white shadow md:h-10 md:w-10" style="-webkit-tap-highlight-color:transparent;-webkit-user-select:none;user-select:none;">
                        <span class="text-2xl leading-none">‹</span>
                    </button>
                    <button type="button" data-related-next aria-label="Siguiente" class="inline-flex h-9 w-9 select-none items-center justify-center rounded-full bg-zinc-900 text-white shadow md:h-10 md:w-10" style="-webkit-tap-highlight-color:transparent;-webkit-user-select:none;user-select:none;">
                        <span class="text-2xl leading-none">›</span>
                    </button>
                </div>
                    ...
                    @foreach([0, 1] as $loopIndex)
                        @foreach($displayItems as $item)
                            <article class="w-[calc((100vw-7rem)/1.2)] shrink-0 text-center sm:w-[40vw] lg:w-[18.7rem]">
                                <a href="{{ $item['href'] }}" class="block {{ $item['href'] === '#' ? 'cursor-default' : '' }}">
                                    <div class="aspect-[16/7] overflow-hidden">
                                        <img src="{{ $item['image'] ?: $placeholderImage }}" alt="{{ e($item['name']) }}" loading="lazy" class="h-full w-full object-contain" />
                                    </div>
                                    <h3 class="mt-4 text-[2rem] font-semibold leading-tight text-zinc-800 sm:text-[1.95rem]">{{ $item['name'] }}</h3>
                                </a>
                            </article>
                        @endforeach
                    @endforeach
...
@endphp
<div id="buyPanel"
     data-base-price="{{ (float) $precioBase }}"
     data-currency-label="{{ $moneda }}"
     data-checkout-action="{{ $checkoutActionPanel }}"
    ...
    data-lens-options-progresivos='@json($lensTypeOptionsProgresivos)'
    data-lens-options-monofocal='@json($lensTypeOptionsMonofocal)'
    data-lens-options-bifocal='@json($lensTypeOptionsBifocal)'
    data-lens-options-ocupacional='@json($lensTypeOptionsOcupacional)'
     data-selected-lens="{{ e((string) $selectedLensType) }}"
     data-selected-nara="{{ e((string) $selectedNaraLevel) }}"
    ...
    <div class="gafa-safari-soft-surface pointer-events-auto ml-auto flex h-full w-full max-w-none flex-col border-l border-zinc-200 bg-white/95 shadow-xl backdrop-blur">
        <div class="flex items-center justify-between px-8 py-4 border-b border-zinc-200/70 bg-zinc-50/70">
        ...
        <div class="flex-1 overflow-y-auto px-8 py-5 space-y-5">
            <div class="rounded-2xl border border-zinc-200/70 bg-white p-4" data-buy-step="1">
                ...
                <div class="mt-3 grid gap-2">
                    <label class="flex cursor-pointer items-start gap-3 rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-900 hover:bg-zinc-50">
                        <input type="radio" name="gafitas_formula" value="no_sin_formula" form="buyPanelPayForm" class="mt-1 h-4 w-4 rounded border-zinc-300" data-buy-step1 required {{ in_array(old('gafitas_formula'), ['no_sin_formula', 'sin_aumento_neutro'], true) ? 'checked' : '' }} />
                        <span class="min-w-0">
                    ...
                    <label class="flex cursor-pointer items-start gap-3 rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-900 hover:bg-zinc-50">
                        <input type="radio" name="gafitas_formula" value="si_con_formula" form="buyPanelPayForm" class="mt-1 h-4 w-4 rounded border-zinc-300" data-buy-step1 required {{ old('gafitas_formula') === 'si_con_formula' ? 'checked' : '' }} />
                        <span class="min-w-0">
            ...
            <div class="rounded-2xl border border-zinc-200/70 bg-white p-4 hidden" data-buy-step="2">
                ...
                <div class="mt-4 space-y-3">
                    ...
                    @endphp
                    @foreach($step2LensModules as $module)
                        <label class="block cursor-pointer">
                            <input
                                ...
                                data-buy-step2
                                required
                                {{ old('tipo_lente_necesitas') === $module['value'] ? 'checked' : '' }}
                            />

                            <div class="relative flex items-center gap-4 overflow-hidden rounded-[1.7rem] border border-zinc-300/80 bg-white p-4 transition duration-200 hover:-translate-y-0.5 hover:border-[#3B7C82] hover:shadow-md peer-checked:border-[#3B7C82] peer-checked:bg-[#3B7C82]/10 peer-checked:shadow-md sm:gap-5 sm:p-5">
                                <div class="flex h-20 w-24 shrink-0 items-center justify-center sm:h-24 sm:w-28">
                                    <img src="{{ $moduleIconFor($module['value']) }}" alt="Lente" loading="lazy" class="h-16 w-20 object-contain sm:h-20 sm:w-24" />
                                </div>

                                <div class="min-w-0 flex-1">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <span class="text-lg font-black tracking-tight text-zinc-900 sm:text-[1.15rem]">{{ $module['title'] }}</span>
                                    </div>
                                    <p class="mt-1.5 text-sm leading-6 text-zinc-600" style="text-align: justify;">{{ $module['description'] }}</p>
                                </div>

                                <span class="absolute right-4 top-4 inline-flex h-5 w-5 items-center justify-center rounded-full border border-zinc-300 bg-white transition peer-checked:border-[#3B7C82] peer-checked:bg-[#3B7C82]">
                ...
                <div class="mt-7 hidden border-t border-zinc-100 pt-5" data-step2-material-wrap>
                    ...
                    <div class="mt-4 space-y-3">
                        @foreach($step2MaterialModules as $module)
                            <button
                                type="button"
                                class="group block w-full rounded-[1.7rem] border border-zinc-300/80 bg-white text-left transition duration-200 hover:-translate-y-0.5 hover:border-[#3B7C82] hover:shadow-md {{ $selectedStep2Material === $module['value'] ? 'ring-2 ring-[#3B7C82] ring-offset-2 ring-offset-white' : '' }}"
                                data-step2-material-card="{{ $module['value'] }}"
                                aria-pressed="{{ $selectedStep2Material === $module['value'] ? 'true' : 'false' }}"
                            >
                                <div class="flex items-center gap-4 p-4 sm:gap-5 sm:p-5">
                                    <div class="flex h-20 w-24 shrink-0 items-center justify-center sm:h-24 sm:w-28">
                                        <img src="{{ asset('images/material.png') }}" alt="Material del lente" loading="lazy" class="h-16 w-20 object-contain sm:h-20 sm:w-24" />
                                    </div>

                                    <div class="min-w-0 flex-1">
                                        <p class="text-[1.05rem] font-black tracking-tight text-zinc-900 sm:text-[1.12rem]">{{ $module['title'] }}</p>
                                        <p class="mt-1.5 text-sm leading-6 text-zinc-600">{{ $module['description'] }}</p>
                                    </div>
                                </div>
                            </button>
                        @endforeach
                ...
                <div class="mt-7 hidden border-t border-zinc-100 pt-5" data-step2-protection-wrap>
                    ...
                    <div class="mt-4 space-y-3">
                        @foreach($step2ProtectionModules as $module)
                            <button
                                type="button"
                                class="group block w-full rounded-[1.7rem] border border-zinc-300/80 bg-white text-left transition duration-200 hover:-translate-y-0.5 hover:border-[#3B7C82] hover:shadow-md {{ $selectedStep2Protection === $module['value'] ? 'ring-2 ring-[#3B7C82] ring-offset-2 ring-offset-white' : '' }}"
                                data-step2-protection-card="{{ $module['value'] }}"
                                aria-pressed="{{ $selectedStep2Protection === $module['value'] ? 'true' : 'false' }}"
                            >
                                <div class="flex items-center gap-4 p-4 sm:gap-5 sm:p-5">
                                    <div class="flex h-20 w-24 shrink-0 items-center justify-center sm:h-24 sm:w-28">
                                        <img src="{{ asset('images/material.png') }}" alt="Material del lente" loading="lazy" class="h-16 w-20 object-contain sm:h-20 sm:w-24" />
            ...
            <div class="rounded-2xl border border-zinc-200/70 bg-white p-4 hidden" data-buy-step="3">
                ...
                <div class="mt-4 space-y-4">
                    <div class="rounded-2xl border border-zinc-200 bg-zinc-50/60 p-4">
                        ...
                        <div class="grid gap-3 sm:grid-cols-2">
                            <label class="block">
                                ...
                                <input type="hidden" name="rx_od_esfera" form="buyPanelPayForm" value="{{ old('rx_od_esfera', '') }}">
                                <button type="button" class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-zinc-300 flex items-center justify-between gap-2 text-left" data-rx-picker data-rx-picker-for="rx_od_esfera" data-rx-picker-type="esfera" data-rx-picker-eye="derecho (OD)">
                                    <span>{{ old('rx_od_esfera') ?: 'Seleccionar' }}</span>
                            ...
                            <label class="block">
                                ...
                                <input type="hidden" name="rx_od_cilindro" form="buyPanelPayForm" value="{{ old('rx_od_cilindro', '') }}">
                                <button type="button" class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-zinc-300 flex items-center justify-between gap-2 text-left" data-rx-picker data-rx-picker-for="rx_od_cilindro" data-rx-picker-type="cilindro" data-rx-picker-eye="derecho (OD)">
                                    <span>{{ old('rx_od_cilindro') ?: 'Seleccionar' }}</span>
                                    <svg class="h-4 w-4 flex-shrink-0 text-zinc-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                                </button>
                            </label>
                            <label class="block">
                                <span class="mb-1 block text-[11px] font-semibold text-zinc-600">Eje</span>
                                <input type="hidden" name="rx_od_eje" form="buyPanelPayForm" value="{{ old('rx_od_eje', '') }}">
                                <button type="button" class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-zinc-300 flex items-center justify-between gap-2 text-left" data-rx-picker data-rx-picker-for="rx_od_eje" data-rx-picker-type="eje" data-rx-picker-eye="derecho (OD)">
                                    <span>{{ old('rx_od_eje') ?: 'Seleccionar' }}</span>
                                    <svg class="h-4 w-4 flex-shrink-0 text-zinc-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                                </button>
                            </label>
                        </div>
                    ...
                    <div class="rounded-2xl border border-zinc-200 bg-zinc-50/60 p-4">
                        ...
                        <div class="grid gap-3 sm:grid-cols-2">
                            <label class="block">
                                <span class="mb-1 block text-[11px] font-semibold text-zinc-600">Esfera</span>
                                <input type="hidden" name="rx_oi_esfera" form="buyPanelPayForm" value="{{ old('rx_oi_esfera', '') }}">
                                <button type="button" class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-zinc-300 flex items-center justify-between gap-2 text-left" data-rx-picker data-rx-picker-for="rx_oi_esfera" data-rx-picker-type="esfera" data-rx-picker-eye="izquierdo (OI)">
                                    <span>{{ old('rx_oi_esfera') ?: 'Seleccionar' }}</span>
                                    <svg class="h-4 w-4 flex-shrink-0 text-zinc-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                                </button>
                            </label>
                            <label class="block">
                                <span class="mb-1 block text-[11px] font-semibold text-zinc-600">Cilindro</span>
                                <input type="hidden" name="rx_oi_cilindro" form="buyPanelPayForm" value="{{ old('rx_oi_cilindro', '') }}">
                                <button type="button" class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-zinc-300 flex items-center justify-between gap-2 text-left" data-rx-picker data-rx-picker-for="rx_oi_cilindro" data-rx-picker-type="cilindro" data-rx-picker-eye="izquierdo (OI)">
                                    <span>{{ old('rx_oi_cilindro') ?: 'Seleccionar' }}</span>
                                    <svg class="h-4 w-4 flex-shrink-0 text-zinc-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                                </button>
                            </label>
                            <label class="block">
                                <span class="mb-1 block text-[11px] font-semibold text-zinc-600">Eje</span>
                                <input type="hidden" name="rx_oi_eje" form="buyPanelPayForm" value="{{ old('rx_oi_eje', '') }}">
                                <button type="button" class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-zinc-300 flex items-center justify-between gap-2 text-left" data-rx-picker data-rx-picker-for="rx_oi_eje" data-rx-picker-type="eje" data-rx-picker-eye="izquierdo (OI)">
                                    <span>{{ old('rx_oi_eje') ?: 'Seleccionar' }}</span>
                                    <svg class="h-4 w-4 flex-shrink-0 text-zinc-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                                </button>
                            </label>
                        </div>
                ...
                <div class="mt-4" data-rx-adicion-wrap>
                    <label class="block">
                        ...
                        <input type="hidden" name="rx_od_adicion" form="buyPanelPayForm" value="{{ old('rx_od_adicion', old('rx_oi_adicion', '')) }}">
                        <input type="hidden" name="rx_oi_adicion" form="buyPanelPayForm" value="{{ old('rx_oi_adicion', old('rx_od_adicion', '')) }}">
                        <button type="button" class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-zinc-300 flex items-center justify-between gap-2 text-left" data-rx-picker data-rx-picker-for="rx_od_adicion" data-rx-picker-type="adicion" data-rx-picker-eye="derecho e izquierdo">
                            <span>{{ old('rx_od_adicion', old('rx_oi_adicion')) ?: 'Seleccionar' }}</span>
                            <svg class="h-4 w-4 flex-shrink-0 text-zinc-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                        </button>
                    </label>
                </div>
                ...
                <div class="mt-4 grid gap-3 sm:grid-cols-2">
                    <div>
                        ...
                        <input name="rx_distancia_pupilar" value="{{ old('rx_distancia_pupilar', old('rx_od_dnp', old('rx_oi_dnp', ''))) }}" form="buyPanelPayForm" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-400" inputmode="decimal" min="0" data-buy-step3-dnp />
                        @error('rx_distancia_pupilar')
                            <p class="mt-2 text-xs font-semibold text-rose-700">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-zinc-700">Año de nacimiento</label>
                        <input name="rx_ano_nacimiento" value="{{ old('rx_ano_nacimiento') }}" form="buyPanelPayForm" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-400" data-buy-step3-year />
                        @error('rx_ano_nacimiento')
                            <p class="mt-2 text-xs font-semibold text-rose-700">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
            ...
            <div class="space-y-5 hidden" data-buy-step="4">
                <div class="rounded-2xl border border-zinc-200/70 bg-zinc-50/60 p-4">
                    <p class="text-sm font-semibold text-zinc-900">{{ $producto->nombre }}</p>
                ...
                <div class="space-y-3 hidden" data-buy-bifocal-cards-wrap>
                    ...
                    <div class="space-y-3">
                        @foreach($bifocalDisplayCards as $key => $card)
                            @php
                                $icon = $resolveFormulaCardIcon($key);
                            @endphp
                            <button
                                type="button"
                                data-buy-bifocal-lens="{{ $key }}"
                                aria-pressed="{{ (string) $selectedLensType === (string) $key ? 'true' : 'false' }}"
                                ...
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                                    @if(!empty($icon))
                                        <div class="hidden sm:flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-full border border-zinc-300 bg-zinc-50">
                                            <img src="{{ $icon }}" alt="{{ $card['title'] }}" class="h-10 w-10 object-contain" loading="lazy">
                                        </div>
                                    @else
                                    ...
                                    @endif
                                    <div class="min-w-0 flex-1">
                                        <p class="text-lg font-bold leading-tight text-zinc-900">{{ $card['title'] }}</p>
                                        @if(filled($card['description']))
                                            <p class="mt-1 text-sm leading-6 text-zinc-700" style="text-align: justify;">{{ $card['description'] }}</p>
                                        @else
                ...
                <div class="space-y-2 hidden" data-buy-ocupacional-wrap>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <button type="button" data-buy-ocupacional="156_blue_block" class="rounded-2xl border border-[#3B7C82] bg-[#3B7C82] px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#326a70]">
                            AR azul
                        </button>
                        <button type="button" data-buy-ocupacional="156_ar_verde" class="rounded-2xl border border-[#3B7C82] bg-[#3B7C82] px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#326a70]">
                            AR Verde
                        </button>
                    </div>
                </div>
                ...
                <div class="space-y-3 hidden" data-buy-monofocal-wrap>
                                ...
                                <div class="grid gap-3 md:grid-cols-[96px_minmax(0,1fr)_minmax(0,1fr)] md:items-center">
                                    ...
                                    <label class="block">
                                        ...
                                        <select name="plano_rx_od_esfera" form="buyPanelPayForm" class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-zinc-400" data-buy-plano-rx-manual="sphere_od">
                                            @foreach($planoNeutralSphereOptions as $option)
                                                <option value="{{ $option['value'] }}" {{ old('plano_rx_od_esfera', '0.00') === $option['value'] ? 'selected' : '' }}>{{ $option['label'] }}</option>
                                            @endforeach
                                        </select>
                                    </label>
                                    <label class="block">
                                        ...
                                        <select name="plano_rx_od_cilindro" form="buyPanelPayForm" class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-zinc-400" data-buy-plano-rx-manual="cyl_od">
                                            @foreach($planoNeutralCylinderOptions as $option)
                                                <option value="{{ $option['value'] }}" {{ old('plano_rx_od_cilindro', '0.00') === $option['value'] ? 'selected' : '' }}>{{ $option['label'] }}</option>
                                            @endforeach
                                        </select>
                                    </label>
                                </div>
                                ...
                                <div class="grid gap-3 md:grid-cols-[96px_minmax(0,1fr)_minmax(0,1fr)] md:items-center">
                                    ...
                                    <label class="block">
                                        ...
                                        <select name="plano_rx_oi_esfera" form="buyPanelPayForm" class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-zinc-400" data-buy-plano-rx-manual="sphere_oi">
                                            @foreach($planoNeutralSphereOptions as $option)
                                                <option value="{{ $option['value'] }}" {{ old('plano_rx_oi_esfera', '0.00') === $option['value'] ? 'selected' : '' }}>{{ $option['label'] }}</option>
                                            @endforeach
                                        </select>
                                    </label>
                                    <label class="block">
                                        ...
                                        <select name="plano_rx_oi_cilindro" form="buyPanelPayForm" class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-zinc-400" data-buy-plano-rx-manual="cyl_oi">
                                            @foreach($planoNeutralCylinderOptions as $option)
                                                <option value="{{ $option['value'] }}" {{ old('plano_rx_oi_cilindro', '0.00') === $option['value'] ? 'selected' : '' }}>{{ $option['label'] }}</option>
                                            @endforeach
                                        </select>
                                    </label>
                                </div>
                    ...
                    <div class="grid gap-2">
                        <button type="button" class="w-full rounded-2xl border border-[#3B7C82] bg-[#3B7C82] px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#326a70]" data-buy-monofocal-category="nara_basica_156">{{ $polyEnabled ? 'Nara POLY 1.59' : 'Nara Basica 1.56' }}</button>
                        <button type="button" class="w-full rounded-2xl border border-[#3B7C82] bg-[#3B7C82] px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#326a70]" data-buy-monofocal-category="nara_premium_160">Nara Premium 1.60</button>
                        <button type="button" class="w-full rounded-2xl border border-[#3B7C82] bg-[#3B7C82] px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#326a70]" data-buy-monofocal-category="alto_indice">Alto indice</button>
                        <button type="button" class="w-full rounded-2xl border border-[#3B7C82] bg-[#3B7C82] px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#326a70]" data-buy-monofocal-category="transition">Transition</button>
                    </div>
                    ...
                    <div class="hidden rounded-2xl border border-zinc-200/70 bg-white p-4" data-buy-monofocal-options="nara_basica_156">
                        ...
                        <div class="mt-3 space-y-3">
                            @foreach($naraBasicaDisplayCards as $key => $card)
                                @php
                                    $icon = $resolveFormulaCardIcon($key);
                                @endphp
                                <button
                                    type="button"
                                    data-buy-monofocal-lens="{{ $key }}"
                                    aria-pressed="{{ (string) $selectedLensType === (string) $key ? 'true' : 'false' }}"
                                    class="w-full rounded-[30px] border border-zinc-300 bg-white p-4 text-left shadow-sm transition hover:border-zinc-400 hover:bg-zinc-50"
                                >
                                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <div class="hidden rounded-2xl border border-zinc-200/70 bg-white p-4" data-buy-monofocal-options="nara_premium_160">
                        <p class="text-xs font-semibold text-zinc-700">Seleccionar</p>
                        <div class="mt-3 space-y-3">
                            @foreach($naraPremiumDisplayCards as $key => $card)
                                @php
                                    $icon = $resolveFormulaCardIcon($key);
                                @endphp
                                <button
                    ...
                    <div class="hidden rounded-2xl border border-zinc-200/70 bg-white p-4" data-buy-monofocal-options="alto_indice">
                        <div class="space-y-4">
                            @foreach($altoIndiceDisplayCards as $sectionTitle => $cards)
                                <div>
                                    <p class="text-sm font-semibold text-zinc-900">{{ $sectionTitle }}</p>
                    ...
                    <div class="hidden rounded-2xl border border-zinc-200/70 bg-white p-4" data-buy-monofocal-options="transition">
                        <button type="button" class="w-full rounded-2xl border border-[#3B7C82] bg-[#3B7C82] px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#326a70]" data-buy-monofocal-lens="159_transitions_gens">Seleccionar</button>
                    </div>
                </div>
                ...
                <div class="space-y-3 hidden" data-buy-noformula-wrap>
                    <div class="rounded-2xl border border-zinc-200/70 bg-white p-4">
                        ...
                        @endphp
                        <div class="mt-3 grid gap-2">
                            @foreach($noFormulaButtonOrder as $noFormulaKey)
                                <button type="button" class="w-full rounded-2xl border border-[#3B7C82] bg-[#3B7C82] px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#326a70]" data-buy-noformula-lens="{{ $noFormulaKey }}">{{ $noFormulaButtonLabels[$noFormulaKey] ?? ($lensTypeOptionsMonofocal[$noFormulaKey] ?? $noFormulaKey) }}</button>
                            @endforeach
                        </div>
                    </div>
                </div>
...
@endphp
                    <select id="buyPanelLensColor" class="w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-800 outline-none focus:border-zinc-400" data-buy-lens-color>
                        ...
                        <option value="Gris" {{ $selectedLensColor === 'Gris' ? 'selected' : '' }}>Gris</option>
                        <option value="Marrón" {{ $selectedLensColor === 'Marrón' ? 'selected' : '' }}>Marrón</option>
                        <option value="Verde Grafito" {{ $selectedLensColor === 'Verde Grafito' ? 'selected' : '' }}>Verde Grafito</option>
                        <option value="Verde esmeralda" {{ $selectedLensColor === 'Verde esmeralda' ? 'selected' : '' }}>Verde esmeralda</option>
                        <option value="Zafiro" {{ $selectedLensColor === 'Zafiro' ? 'selected' : '' }}>Zafiro</option>
                        <option value="Amatista" {{ $selectedLensColor === 'Amatista' ? 'selected' : '' }}>Amatista</option>
                        <option value="Ambar" {{ $selectedLensColor === 'Ambar' ? 'selected' : '' }}>Ambar</option>
                        <option value="Rubi" {{ $selectedLensColor === 'Rubi' ? 'selected' : '' }}>Rubi</option>
                    </select>
                ...
                <div class="space-y-3" data-buy-nara-wrap>
                    ...
                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                        <button type="button" data-buy-nara="basica" class="group flex flex-col items-center rounded-2xl border border-zinc-200/70 bg-white px-3 py-3 text-center text-xs font-semibold text-zinc-700 shadow-sm transition hover:border-[#3B7C82] hover:bg-[#3B7C82]/10">
                            <div class="mb-2 h-14 w-16 rounded-full bg-center bg-cover shadow-inner" style="background-image: url('{{ $formulas['nara_basica'] ?? $formulas['default_lente'] }}');"></div>
                        ...
                        <button type="button" data-buy-nara="media" class="group flex flex-col items-center rounded-2xl border border-zinc-200/70 bg-white px-3 py-3 text-center text-xs font-semibold text-zinc-700 shadow-sm transition hover:border-[#3B7C82] hover:bg-[#3B7C82]/10">
                            <div class="mb-2 h-14 w-16 rounded-full bg-center bg-cover shadow-inner" style="background-image: url('{{ $formulas['nara_media'] ?? $formulas['default_lente'] }}');"></div>
                            <span class="text-[11px] font-bold text-sky-900">NARA</span>
                        ...
                        <button type="button" data-buy-nara="alta" class="group flex flex-col items-center rounded-2xl border border-zinc-200/70 bg-white px-3 py-3 text-center text-xs font-semibold text-zinc-700 shadow-sm transition hover:border-[#3B7C82] hover:bg-[#3B7C82]/10">
                            <div class="mb-2 h-14 w-16 rounded-full bg-center bg-cover shadow-inner" style="background-image: url('{{ $formulas['nara_alta'] ?? $formulas['default_lente'] }}');"></div>
                            <span class="text-[11px] font-bold text-sky-900">NARA</span>
                        ...
                        @if(!$polyEnabled)
                        <button type="button" data-buy-nara="premium" class="group flex flex-col items-center rounded-2xl border border-zinc-200/70 bg-white px-3 py-3 text-center text-xs font-semibold text-zinc-700 shadow-sm transition hover:border-[#3B7C82] hover:bg-[#3B7C82]/10">
                            <div class="mb-2 h-14 w-16 rounded-full bg-center bg-cover shadow-inner" style="background-image: url('{{ $formulas['nara_premium'] ?? $formulas['default_lente'] }}');"></div>
                            <span class="text-[11px] font-bold text-sky-900">NARA</span>
...
@endphp
                <div class="space-y-4 hidden" data-buy-progresivos-presets>
                    ...
                    <div class="rounded-2xl border border-[#3B7C82] bg-[#3B7C82] p-4 text-white" data-buy-progresivo-card="156_blanco">
                        <p class="text-xs font-semibold text-white" data-buy-progresivo-card-title>{{ $polyEnabled ? 'Nara POLY 1.59 Blanco' : 'Lente Nara Basico 1.56' }}</p>
                        <p class="mt-2 text-xs text-white/90">{{ $polyEnabled ? 'Lente progresivo Nara POLY 1.59 blanco. Permite ver de lejos, intermedio y de cerca en un solo lente.' : 'Lente progresivo blanco 1.56, la opción más sencilla. Permite ver de lejos, intermedio y de cerca en un solo lente. Ideal si buscas una solución funcional y económica' }}</p>
                        <button type="button" class="mt-3 w-full rounded-2xl border border-white/70 bg-white px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-[#3B7C82] transition hover:bg-white/90" data-buy-lens-preset="156_blanco">SELECCIONAR</button>
                    </div>

                    <div class="rounded-2xl border border-[#3B7C82] bg-[#3B7C82] p-4 text-white" data-buy-progresivo-card="156_fotocromatico_superhidrofobico">
                        <p class="text-xs font-semibold text-white" data-buy-progresivo-card-title>{{ $polyEnabled ? 'Nara POLY 1.59 Fotocromatico' : 'Lente nara 1.56' }}</p>
                        <p class="mt-2 text-xs text-white/90">{{ $polyEnabled ? 'Lente progresivo Nara POLY 1.59 fotocromático: claro en interiores y oscuro al sol. Incluye antirreflejo, filtro azul y capa hidrofóbica.' : 'Lente progresivo fotocromático que se adapta a la luz: claro en interiores y oscuro al sol. Incluye antireflejo, filtro de luz azul y capa hidrofóbica, que ayuda a reducir reflejos, proteger tus ojos y mantener el lente más limpio' }}</p>
                        <button type="button" class="mt-3 w-full rounded-2xl border border-white/70 bg-white px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-[#3B7C82] transition hover:bg-white/90" data-buy-lens-preset="156_fotocromatico_superhidrofobico">SELECCIONAR</button>
                    </div>

                    <div class="rounded-2xl border border-[#3B7C82] bg-[#3B7C82] p-4 text-white" data-buy-progresivo-card="160_premium">
                        ...
                        <button type="button" class="mt-3 w-full rounded-2xl border border-white/70 bg-white px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-[#3B7C82] transition hover:bg-white/90" data-buy-lens-preset="160_premium">SELECCIONAR</button>
                    </div>

                    <div class="rounded-2xl border border-[#3B7C82] bg-[#3B7C82] p-4 text-white" data-buy-progresivo-card="159_transitions_gens">
                        <p class="text-xs font-semibold text-white" data-buy-progresivo-card-title>{{ $polyEnabled ? 'Nara POLY 1.59 Transitions' : 'Transition' }}</p>
                        ...
                        <button type="button" class="mt-3 w-full rounded-2xl border border-white/70 bg-white px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-[#3B7C82] transition hover:bg-white/90" data-buy-lens-preset="159_transitions_gens">SELECCIONAR</button>
                    </div>
                </div>
                ...
                <div class="rounded-2xl border border-zinc-200/70 bg-white p-4" data-buy-pdf-wrap>
                    ...
                    <input type="file" name="prescription_pdf" form="buyPanelPayForm" accept="application/pdf" class="mt-3 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-2.5 text-xs text-zinc-700 file:mr-3 file:rounded-xl file:border-0 file:bg-[#3B7C82] file:px-3 file:py-2 file:text-xs file:font-semibold file:text-white hover:file:bg-[#326a70]" data-buy-pdf-input />
        ...
        <div class="border-t border-zinc-200/70 bg-white/90 px-6 py-4 space-y-3">
            ...
            <form id="buyPanelPayForm" action="{{ route('gafas.prescription.checkout', ['producto' => $producto->slug]) }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                @csrf
                ...
                <input type="hidden" name="plano_rx_sphere_max" value="{{ e((string) old('plano_rx_sphere_max', request('plano_rx_sphere_max', '0.00'))) }}" data-buy-plano-rx-sphere />
                <input type="hidden" name="plano_rx_cyl_max" value="{{ e((string) old('plano_rx_cyl_max', request('plano_rx_cyl_max', '0.00'))) }}" data-buy-plano-rx-cyl />
                <input type="hidden" name="lens_type" value="{{ e((string) $selectedLensType) }}" data-buy-lens-type-input />
                <input type="hidden" name="nara_level" value="{{ e((string) $selectedNaraLevel) }}" data-buy-nara-input />
                <input type="hidden" name="step2_material" value="{{ e((string) old('step2_material', request('step2_material'))) }}" data-step2-material-input />
                <input type="hidden" name="step2_protection" value="{{ e((string) old('step2_protection', request('step2_protection'))) }}" data-step2-protection-input />
                <input type="hidden" name="frame_color" value="{{ e((string) ($colorLabel ?? ($producto->color ?? ''))) }}" data-buy-frame-color-input />
                <input type="hidden" name="lens_color" value="{{ e((string) old('lens_color', request('lens_color'))) }}" data-buy-lens-color-input {{ (string) $selectedLensType === '159_transitions_gens' ? '' : 'disabled' }} />
                <button type="submit" class="inline-flex w-full items-center justify-center rounded-2xl bg-[#3B7C82] px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-[#326a70] focus:outline-none focus:ring-2 focus:ring-[#3B7C82]">
                    Continuar al pago
                </button>
            </form>
...
<script>
    (() => {
        ...
        const setActive = (activeBtn) => {
            buttons.forEach((b) => {
                b.classList.remove('ring-2', 'ring-[#3B7C82]', 'ring-offset-2', 'ring-offset-white');
                b.setAttribute('aria-current', 'false');
            });
            if (!activeBtn) return;
            activeBtn.classList.add('ring-2', 'ring-[#3B7C82]', 'ring-offset-2', 'ring-offset-white');
        ...
        const renderThumbButtons = (images = [], preferredSrc = '') => {
            ...
            uniqueImages.forEach((src) => {
                ...
                button.type = 'button';
                button.className = 'group w-16 shrink-0 overflow-hidden rounded-2xl border border-zinc-200/70 bg-white shadow-sm transition focus:outline-none focus:ring-2 focus:ring-[#3B7C82] sm:w-auto';
    ...
    (() => {
        ...
        const setActiveOcupacional = () => {
            ...
            ocupacionalButtons.forEach((btn) => {
                btn.classList.remove('ring-2', 'ring-[#3B7C82]', 'ring-offset-2', 'ring-offset-white');
                btn.setAttribute('aria-pressed', 'false');
            });
            const active = ocupacionalButtons.find((btn) => String(btn.getAttribute('data-buy-ocupacional') || '') === String(selectedLensType || ''));
            if (!active) return;
            active.classList.add('ring-2', 'ring-[#3B7C82]', 'ring-offset-2', 'ring-offset-white');
        ...
        const setActiveStep2Card = (cards, activeValue) => {
            cards.forEach((card) => {
                card.classList.remove('ring-2', 'ring-[#3B7C82]', 'ring-offset-2', 'ring-offset-white');
        ...
        const syncNoFormulaLensButtons = () => {
            ...
            noFormulaLensButtons.forEach((button) => {
                button.classList.remove('ring-2', 'ring-[#3B7C82]', 'ring-offset-2', 'ring-offset-white');
    ...
    <div class="absolute inset-x-0 bottom-0 sm:inset-0 sm:flex sm:items-center sm:justify-center sm:p-6 pointer-events-none">
        <div class="pointer-events-auto relative w-full sm:max-w-2xl rounded-t-3xl sm:rounded-3xl bg-white shadow-2xl flex flex-col" style="max-height:88svh;max-height:88dvh">
            ...
            <div class="px-4 sm:px-6 py-4 space-y-2 border-t border-zinc-100 flex-shrink-0">
                <button type="button" id="rxPickerConfirm" class="w-full rounded-2xl bg-[#3B7C82] py-3.5 text-sm font-bold text-white hover:bg-[#326a70] active:bg-[#2b5b60] transition">Confirmar</button>
                <button type="button" id="rxPickerCancel" class="w-full py-2.5 text-sm font-semibold text-[#3B7C82] hover:text-[#326a70] transition">Cancelar</button>
            </div>
        </div>
    </div>
</div>
...
(function () {
    ...
    function btnCls(selected) {
        return 'rounded-xl border px-2 py-2 text-xs font-semibold text-center transition cursor-pointer ' +
            (selected ? 'border-[#3B7C82] bg-[#3B7C82] text-white' : 'border-zinc-200 bg-white text-zinc-800 hover:border-[#3B7C82] hover:bg-[#3B7C82]/10');
    }
