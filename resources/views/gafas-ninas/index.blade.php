...
<head>
    ...
    <title>Gafas niñas — Óptica</title>
...
<body class="bg-zinc-50 text-zinc-900 antialiased font-sans overflow-x-hidden">
@php
...
<main class="mx-auto max-w-7xl px-4 py-8">
    <div class="mb-6 text-center">
        ...
        <h1 class="mt-1 text-3xl font-extrabold tracking-tight text-zinc-900 sm:text-4xl">Gafas niñas</h1>
        <p class="mt-2 text-base text-zinc-500">Colección para niñas</p>
    </div>
                ...
                <form method="GET" action="{{ url()->current() }}" class="mt-4">
                    ...
                    <div class="mt-3 grid gap-3">
                        <div data-price-filter-form>
                            <div class="grid gap-3">
                                <div>
                                    ...
                                    <input id="priceMin" type="range" name="min_price" min="0" max="{{ $sliderMax }}" step="{{ $sliderStep }}" value="{{ $sliderMinValue }}" class="mt-3 w-full accent-zinc-900" aria-label="Filtrar por precio mínimo" />
                                </div>
                                <div>
                                    <div class="flex items-center justify-between gap-3">
                                    ...
                                    <input id="priceMax" type="range" name="max_price" min="0" max="{{ $sliderMax }}" step="{{ $sliderStep }}" value="{{ $sliderMaxValue }}" class="mt-3 w-full accent-zinc-900" aria-label="Filtrar por precio máximo" />
                                </div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <button type="submit" data-loading class="inline-flex items-center justify-center rounded-2xl bg-zinc-950 px-4 py-3 text-sm font-semibold text-white hover:bg-zinc-900">Aplicar</button>
                            <a href="{{ url()->current() }}" class="inline-flex items-center justify-center rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm font-semibold text-zinc-900 hover:bg-zinc-50">Limpiar</a>
                        </div>
                    </div>
                </form>
        ...
        <section class="relative">
            ...
            @else
                <div class="mb-4 flex items-center justify-between gap-3">
                    <p class="text-sm font-semibold text-zinc-700">Mostrando <span class="text-zinc-900">{{ $productos->count() }}</span> de <span class="text-zinc-900">{{ $productos->total() }}</span></p>
                </div>

                <div class="grid grid-cols-3 gap-2 sm:gap-4 sm:grid-cols-3 xl:grid-cols-3">
                    @foreach($productos as $producto)
                        @php
                            ...
                            if (count($swatches) === 0) {
                                $swatches = ['#2f2329', '#8f7660'];
                                $colorVariants = [
                                    ['name' => 'Color', 'hex' => '#2f2329', 'image' => ''],
                                    ['name' => 'Color', 'hex' => '#8f7660', 'image' => ''],
                                ];
                            } elseif (count($swatches) === 1) {
                                $swatches[] = '#8f7660';
                                $colorVariants[] = ['name' => 'Color', 'hex' => '#8f7660', 'image' => ''];
                            }
