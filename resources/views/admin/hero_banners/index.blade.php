...
@section('content')
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <a href="{{ route('configuracion.hero-banners.create') }}" class="inline-flex items-center justify-center rounded-2xl bg-zinc-900 px-5 py-3 text-sm font-semibold text-white hover:bg-zinc-800">Nuevo banner</a>
    </div>

    <form method="POST" action="{{ route('configuracion.hero-banners.settings.update') }}" class="mt-4 rounded-2xl border border-zinc-200 bg-white p-4">
        @csrf
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <label for="seconds_per_slide" class="text-sm font-semibold text-zinc-800">Cambiar imagen cada (segundos)</label>
                <input id="seconds_per_slide" name="seconds_per_slide" type="number" min="2" max="30" value="{{ old('seconds_per_slide', (int)($heroCarousel['seconds_per_slide'] ?? 5)) }}" class="mt-2 w-32 rounded-xl border border-zinc-300 px-3 py-2 text-sm">
            </div>
            ...
            @forelse($banners as $banner)
                ...
                <tr>
                    ...
                    <td class="px-4 py-3">
                        ...
                        <p class="mt-2 text-xs text-zinc-500">{{ $banner->uploaded_image_original_name ?: 'Imagen de banner' }}</p>
                    </td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('configuracion.hero-banners.edit', $banner) }}" class="rounded-xl px-3 py-2 text-sm font-semibold text-zinc-700 ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50">Editar</a>
                            <form method="POST" action="{{ route('configuracion.hero-banners.destroy', $banner) }}" onsubmit="return confirm('¿Eliminar este banner?');">
                                @csrf
                                @method('DELETE')
