...
@php($imgUrl = $banner ? $banner->heroImageUrl($placeholderImg) : $placeholderImg)
@php($isEdit = !is_null($banner))

<section class="rounded-3xl border border-zinc-200 bg-white p-6">
    <label class="text-sm font-semibold">{{ $isEdit ? 'Imagen del banner' : 'Imágenes de banners' }}</label>
    <input
    ...
    @if(!$isEdit)
        <p id="heroBannerFilesCount" class="mt-1 text-xs text-zinc-500"></p>
    @endif

    <div class="mt-5 overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-100">
        <img id="heroBannerImagePreview" src="{{ $imgUrl }}" alt="Vista previa banner" class="h-auto w-full object-cover" loading="lazy">
    </div>
    ...
    @if($isEdit)
        <div class="mt-6 border-t border-zinc-200 pt-5">
            ...
            <p id="heroBannerExtraCount" class="mt-1 text-xs text-zinc-500"></p>
        </div>
    @endif
</section>
...
<script>
    (() => {
        ...
        const preview = document.getElementById('heroBannerImagePreview');
        const filesCount = document.getElementById('heroBannerFilesCount');
        const extraInput = document.getElementById('heroBannerExtraFiles');
        const extraCount = document.getElementById('heroBannerExtraCount');
