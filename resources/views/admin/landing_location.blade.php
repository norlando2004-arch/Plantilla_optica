...
@section('content')
    <div class="flex items-end justify-between">
        ...
        <a href="{{ url('/') }}#ubicacion" class="rounded-xl px-3 py-2 text-sm font-semibold text-zinc-700 ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50">Ver landing</a>
    </div>

    <div class="mt-6 grid gap-6 lg:grid-cols-12">
        <section class="rounded-3xl border border-zinc-200 bg-white p-6 lg:col-span-12">
            <form id="landingLocationForm" method="POST" action="{{ route('configuracion.landing-location.update') }}" enctype="multipart/form-data">
                @csrf

                <div id="locationCardsWrap" class="grid gap-6 lg:grid-cols-3">
                    @foreach($baseLocations as $i => $loc)
                        <article class="rounded-2xl border-2 border-zinc-300 bg-white p-5 shadow-sm" data-location-card data-index="{{ $i }}">
                            <div class="mb-4 flex items-center justify-between gap-2 border-b-2 border-zinc-300 pb-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <p class="text-sm font-bold text-zinc-800" data-location-title>Sede {{ $i + 1 }}</p>
                                </div>
                            ...
                            <div class="mt-4 grid gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        value="{{ old("locations.$i.name", $loc['name']) }}"
                                ...
                                <div>
                                    ...
                                    >{{ old("locations.$i.description", $loc['description'] ?? '') }}</textarea>
                                ...
                                <div>
                                    ...
                                    <label class="mt-2 inline-flex items-center gap-2 text-xs text-zinc-600">
                                        <input type="checkbox" name="locations[{{ $i }}][remove_image]" value="1" class="rounded border-zinc-300">
                                ...
                                <div class="rounded-xl border border-zinc-200 bg-white p-3">
                                    ...
                                    <div class="mt-3 grid gap-3 sm:grid-cols-3">
                                        <div>
                                            ...
                                            <input data-lat-input data-index="{{ $i }}" name="locations[{{ $i }}][lat]" value="{{ old("locations.$i.lat", $loc['lat']) }}" class="mt-1 w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm" inputmode="decimal">
                                        </div>
                                        <div>
                                            <label class="text-[11px] font-semibold text-zinc-600">Longitud</label>
                                            <input data-lng-input data-index="{{ $i }}" name="locations[{{ $i }}][lng]" value="{{ old("locations.$i.lng", $loc['lng']) }}" class="mt-1 w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm" inputmode="decimal">
                                        </div>
                                        <div>
                                            <label class="text-[11px] font-semibold text-zinc-600">Zoom</label>
                                            <input data-zoom-input data-index="{{ $i }}" name="locations[{{ $i }}][zoom]" value="{{ old("locations.$i.zoom", $loc['zoom']) }}" class="mt-1 w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm" inputmode="numeric">
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        (() => {
            ...
            const baseImages = @json(array_map(fn ($l) => $l['image_url'], $baseLocations));
