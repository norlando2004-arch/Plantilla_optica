...
@section('title', 'Importar gafas polarizadas')
@section('heading', 'Importar gafas polarizadas')

@section('content')
    <div class="mb-6 rounded-3xl border border-zinc-200 bg-white p-5">
        <div class="mb-6">
            <h3 class="text-base font-semibold text-zinc-900">Cargar gafas polarizadas desde Excel</h3>
            <p class="mt-1 text-sm text-zinc-500">Sube un archivo Excel con las gafas polarizadas que deseas agregar.</p>
        </div>

        <form action="{{ route('admin.gafas-polarizadas.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex flex-wrap gap-3">
                <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-zinc-950 px-6 py-2 text-sm font-semibold text-white hover:bg-zinc-900">Importar gafas polarizadas</button>
                <a href="{{ route('admin.gafas-polarizadas.import.template') }}" class="inline-flex items-center justify-center rounded-2xl border border-blue-200 bg-blue-50 px-6 py-2 text-sm font-semibold text-blue-700 hover:bg-blue-100" download>Descargar plantilla</a>
                <a href="{{ route('admin.gafas-polarizadas.index') }}" class="inline-flex items-center justify-center rounded-2xl border border-zinc-200 bg-white px-6 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">Cancelar</a>
            </div>
        </form>
    </div>
@endsection
