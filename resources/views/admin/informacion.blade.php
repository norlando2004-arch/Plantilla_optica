...
@section('heading', 'Información del Dashboard')
...
<div class="space-y-6">
    <div class="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <form method="POST" action="{{ route('admin.informacion.love-cta') }}" class="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
                @csrf
                ...
                <label class="inline-flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm font-medium text-zinc-800">
                    <input type="checkbox" name="hide_love_cta_for_clients" value="1" class="h-4 w-4 rounded border-zinc-300 text-zinc-900 focus:ring-zinc-400" @checked($hideLoveCtaForClients)>
        ...
        <div class="rounded-2xl border border-zinc-200 bg-gradient-to-br from-green-50 to-green-100 p-4 shadow-sm transition hover:shadow-md sm:p-6">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="mt-2 break-words text-2xl font-bold text-green-900 sm:text-3xl">${{ number_format($ingresosTotales, 0) }}</p>
        ...
        <div class="rounded-2xl border border-zinc-200 bg-gradient-to-br from-purple-50 to-purple-100 p-4 shadow-sm transition hover:shadow-md sm:p-6">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="mt-2 break-words text-2xl font-bold text-purple-900 sm:text-3xl">{{ $cantidadPagos }}</p>
        ...
        <div class="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-orange-100 p-4 shadow-sm transition hover:shadow-md sm:p-6">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="mt-2 break-words text-2xl font-bold text-orange-900 sm:text-3xl">${{ number_format($ingresosEstesMes, 0) }}</p>
                    <p class="mt-2 text-xs text-orange-600">{{ now()->format('F Y') }}</p>
                </div>
        ...
        <div class="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-6">
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between gap-3 border-b border-zinc-100 py-2">
                    ...
                    <span class="break-words text-right font-semibold text-zinc-900">${{ $cantidadPagos > 0 ? number_format($ingresosTotales / $cantidadPagos, 2) : '0' }}</span>
                </div>
                <div class="flex items-center justify-between gap-3 border-b border-zinc-100 py-2">
                    <span class="text-sm text-zinc-600">Ingresos este mes</span>
                    <span class="break-words text-right font-semibold text-zinc-900">${{ number_format($ingresosEstesMes, 0) }}</span>
                </div>
                <div class="flex items-center justify-between gap-3 py-2">
                    <span class="text-sm text-zinc-600">Pagos completados</span>
                    <span class="break-words text-right font-semibold text-zinc-900">{{ $cantidadPagos }}</span>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    const mesesNombres = {!! $mesesNombres !!};
    const ingresosPorMesData = {!! $ingresosPorMesData !!};
    const visitantesPorMesData = {!! $visitantesPorMesData !!};
    ...
    new Chart(ctxEstado, {
        type: 'doughnut',
        data: {
            labels: {!! json_encode($estadoPagos->pluck('estado')->toArray()) !!},
            datasets: [{
                data: {!! json_encode($estadoPagos->pluck('cantidad')->toArray()) !!},
                backgroundColor: [
