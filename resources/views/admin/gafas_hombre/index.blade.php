...
@section('title', 'Gafas hombre')
@section('heading', 'Gafas hombre')
...
@section('content')
    ...
    <div class="mb-6 rounded-3xl border border-zinc-200 bg-white p-5">
        ...
        <form class="mt-4" action="{{ route('admin.gafas-hombre.promo-image.update') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
                ...
                <div class="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50">
                    <img src="{{ $promoPreview }}" alt="Vista previa banner hombre" class="h-56 w-full object-cover" loading="lazy">
                </div>
            </div>
                    ...
                    <div class="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
                        @foreach($promoAssets as $promoAsset)
                            ...
                            <div class="relative overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50">
                                ...
                                @endif
                                <img src="{{ $promoImageUrl }}" alt="Banner hombre" class="h-24 w-full object-cover" loading="lazy">
                            </div>
                        @endforeach
                    </div>
                </div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('admin.gafas-hombre.import.show') }}" class="inline-flex items-center justify-center rounded-2xl border border-zinc-200 bg-white px-4 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">
                Importar Excel
            </a>
            <a href="{{ route('admin.gafas-hombre.create') }}" class="inline-flex items-center justify-center rounded-2xl bg-zinc-950 px-4 py-2 text-sm font-semibold text-white hover:bg-zinc-900">
                + Agregar
            </a>
        </div>
    </div>
                    ...
                    <tr class="bg-white">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.gafas-hombre.edit', $producto) }}" class="rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">Editar</a>
                                <form action="{{ route('admin.gafas-hombre.destroy', $producto) }}" method="POST" onsubmit="return confirm('¿Eliminar esta gafa?')">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="mt-6">
        <a href="{{ route('gafas-hombre.index') }}" class="text-sm font-semibold text-zinc-700 hover:underline">Ver página pública: /gafas-hombre</a>
    </div>
