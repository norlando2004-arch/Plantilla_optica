...
@section('title', 'Editar gafa (Hombre)')
...
@section('content')
    ...
    <form class="mt-6" action="{{ route('admin.gafas-hombre.update', $producto) }}" method="POST" enctype="multipart/form-data" data-single-submit>
        @csrf
        @method('PUT')
        <div class="rounded-3xl border border-zinc-200 bg-zinc-50 p-6">
            @include('admin.gafas_mujeres._form', ['producto' => $producto])
