...
<body class="bg-[linear-gradient(180deg,#eef7f8_0%,#e3f1f2_28%,#d5e9eb_60%,#f7fbfb_100%)] text-zinc-900 antialiased font-sans">
...
@php
    ...
    $clipOnRaw = $checkoutCaracteristicas['clip_on_compatible'] ?? $checkoutMeta['clip_on_compatible'] ?? null;
    if (is_string($clipOnRaw)) {
        $clipOnNormalized = strtolower(trim($clipOnRaw));
        if (in_array($clipOnNormalized, ['1', 'si', 'sí', 'true'], true)) {
            $clipOnSummary = 'Sí';
        } elseif (in_array($clipOnNormalized, ['0', 'no', 'false'], true)) {
            $clipOnSummary = 'No';
        } else {
            $clipOnSummary = trim($clipOnRaw) !== '' ? trim($clipOnRaw) : $textFallback;
        }
    } elseif (is_bool($clipOnRaw) || is_numeric($clipOnRaw)) {
        $clipOnSummary = (bool) $clipOnRaw ? 'Sí' : 'No';
    } else {
        $clipOnSummary = 'No';
    }
    $compatProgresivosSummary = isset($checkoutCaracteristicas['progresivos'])
        ? ((bool) $checkoutCaracteristicas['progresivos'] ? 'Sí' : 'No')
        : 'No';
    ...
    $detailRows = [
        ...
        ['label' => 'Ancho total de la montura', 'value' => $frameWidthSummary ?? $textFallback],
        ['label' => 'Ancho del lente', 'value' => $lensWidthSummary ?? $textFallback],
        ['label' => 'Alto del lente', 'value' => $lensHeightSummary ?? $textFallback],
        ['label' => 'Puente', 'value' => $bridgeSummary ?? $textFallback],
        ['label' => 'Largo de patillas', 'value' => $templeSummary ?? $textFallback],
...
<main class="relative mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            <h2 class="mt-0.5 text-lg font-semibold">Tus datos</h2>
        </div>
    ...
    <div class="grid gap-6 lg:grid-cols-12 lg:items-start">
        <section class="lg:col-span-4 lg:sticky lg:top-24">
            <div id="lensBreakdown" class="overflow-hidden rounded-[2rem] bg-white/42 shadow-[0_26px_80px_-48px_rgba(15,23,42,0.45)] ring-1 ring-white/60 backdrop-blur-xl" data-currency="{{ $producto->moneda ?? 'COP' }}" data-base="{{ (float) $precioBase }}">
                ...
                <div class="space-y-5 px-5 pb-5">
                    <div class="rounded-[1.6rem] bg-white/70 p-4 ring-1 ring-white/80">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 text-xs text-zinc-500">{{ $producto->marca ?: 'Óptica' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        @foreach($quickSpecs as $spec)
                            <div class="rounded-2xl bg-white/72 p-3 ring-1 ring-white/90">
                                <div class="mb-2 inline-flex h-6 w-6 items-center justify-center rounded-lg bg-sky-100 text-sky-600">
                    ...
                    <div class="rounded-[1.4rem] bg-white/76 p-4 ring-1 ring-white/90">
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">Detalle del producto</p>
                        <div class="mt-3 divide-y divide-zinc-200 text-sm">
                            @foreach($detailRows as $row)
                                <div class="flex items-start justify-between gap-4 py-2.5">
                                    <span class="text-zinc-500">{{ $row['label'] }}</span>
                                    <span class="text-right font-semibold text-zinc-900">{{ $row['value'] }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="rounded-[1.4rem] bg-white/76 p-4 ring-1 ring-white/90">
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">Compatibilidad</p>
                        <div class="mt-3 divide-y divide-zinc-200 text-sm">
                            <div class="flex items-start justify-between gap-4 py-2.5">
                                <span class="text-zinc-500">Progresivos</span>
                                <span class="text-right font-semibold text-zinc-900">{{ $compatProgresivosSummary }}</span>
                            </div>
                            <div class="flex items-start justify-between gap-4 py-2.5">
                                <span class="text-zinc-500">Tipo de fórmula</span>
                                <span class="text-right font-semibold text-zinc-900">{{ $compatTipoFormulaSummary !== '' ? $compatTipoFormulaSummary : 'Bajas' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <section class="lg:col-span-8 rounded-[2rem] border border-zinc-200 bg-white/92 p-4 shadow-[0_24px_70px_-40px_rgba(15,23,42,0.3)] backdrop-blur sm:p-6">
            ...
            <form id="pagoForm" class="mt-5" action="{{ route('pagos.start', ['producto' => $producto->slug]) }}" method="POST" novalidate>
                @csrf
                ...
                <div class="rounded-2xl border border-zinc-200 bg-white p-4 {{ $noPrescription ? 'hidden' : '' }}" id="lensConfigurator"
                     data-mode="{{ $mode }}"
                    ...
                    <p class="text-sm font-semibold text-zinc-900">
                        {{ $noFormulaSimple ? 'Sin fórmula' : ($mode === 'monofocal' ? 'Lentes monofocales' : ($mode === 'bifocal' ? 'Lentes bifocales' : ($mode === 'ocupacional' ? 'Lentes ocupacionales' : 'Lentes progresivos digitales'))) }}
                    </p>
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2 {{ (string) $selectedLensType === '159_transitions_gens' ? '' : 'hidden' }}" data-lens-color>
                ...
                <div class="mt-5 rounded-2xl border border-amber-300 bg-amber-50/50 p-4 ring-2 ring-amber-200/80" data-profile-new-wrap>
                    <input type="hidden" name="crear_nuevo_perfil" value="1" />
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        <div class="relative sm:col-span-2">
                            <label class="text-xs font-semibold text-zinc-700" for="correo">Correo para enviar factura (opcional)</label>
                            <input id="correo" type="email" name="correo" value="{{ old('correo') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-400 @error('correo') border-amber-400 bg-amber-50 @enderror" />
                            @error('correo')
                                <p class="mt-2 text-xs font-semibold text-amber-700">{{ $message }}</p>
                            @enderror
                        </div>

                        <div class="relative" data-profile-field="nombres">
                            <label class="text-xs font-semibold text-zinc-700">Nombres</label>
                            <input name="nombres" value="{{ old('nombres') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-400 @error('nombres') border-amber-400 bg-amber-50 @enderror" data-profile-input="nombres" />
                            @error('nombres')
                                <p class="mt-2 text-xs font-semibold text-amber-700">{{ $message }}</p>
                            @enderror
                            <p class="mt-2 hidden text-xs font-semibold text-amber-700" data-profile-error="nombres">Falta rellenar este campo.</p>
                        </div>

                        <div class="relative" data-profile-field="apellidos">
                            <label class="text-xs font-semibold text-zinc-700">Apellidos</label>
                            <input name="apellidos" value="{{ old('apellidos') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-400 @error('apellidos') border-amber-400 bg-amber-50 @enderror" data-profile-input="apellidos" />
                            @error('apellidos')
                                <p class="mt-2 text-xs font-semibold text-amber-700">{{ $message }}</p>
                            @enderror
                            <p class="mt-2 hidden text-xs font-semibold text-amber-700" data-profile-error="apellidos">Falta rellenar este campo.</p>
                        </div>

                        <div class="relative" data-profile-field="tipo_documento">
                            <label class="text-xs font-semibold text-zinc-700">Tipo documento</label>
                            <select name="tipo_documento" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-400 @error('tipo_documento') border-amber-400 bg-amber-50 @enderror" data-profile-input="tipo_documento">
                            ...
                            @error('tipo_documento')
                                <p class="mt-2 text-xs font-semibold text-amber-700">{{ $message }}</p>
                            @enderror
                            <p class="mt-2 hidden text-xs font-semibold text-amber-700" data-profile-error="tipo_documento">Falta rellenar este campo.</p>
                        </div>
                        <div class="relative" data-profile-field="numero_documento">
                            <label class="text-xs font-semibold text-zinc-700">Número documento</label>
                            <input name="numero_documento" value="{{ old('numero_documento') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-400 @error('numero_documento') border-amber-400 bg-amber-50 @enderror" data-profile-input="numero_documento" />
                            @error('numero_documento')
                                <p class="mt-2 text-xs font-semibold text-amber-700">{{ $message }}</p>
                            @enderror
                            <p class="mt-2 hidden text-xs font-semibold text-amber-700" data-profile-error="numero_documento">Falta rellenar este campo.</p>
                        </div>
                        <div class="relative" data-profile-field="telefono">
                            <label class="text-xs font-semibold text-zinc-700">Teléfono</label>
                            <input name="telefono" value="{{ old('telefono') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-400 @error('telefono') border-amber-400 bg-amber-50 @enderror" data-profile-input="telefono" />
                            @error('telefono')
                                <p class="mt-2 text-xs font-semibold text-amber-700">{{ $message }}</p>
                            @enderror
                            <p class="mt-2 hidden text-xs font-semibold text-amber-700" data-profile-error="telefono">Falta rellenar este campo.</p>
                        </div>
                        <div class="relative" data-profile-field="ciudad">
                            <label class="text-xs font-semibold text-zinc-700">Ciudad</label>
                            <input name="ciudad" value="{{ old('ciudad') }}" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-400 @error('ciudad') border-amber-400 bg-amber-50 @enderror" data-profile-input="ciudad" />
                            @error('ciudad')
                                <p class="mt-2 text-xs font-semibold text-amber-700">{{ $message }}</p>
                            @enderror
                            <p class="mt-2 hidden text-xs font-semibold text-amber-700" data-profile-error="ciudad">Falta rellenar este campo.</p>
                        </div>
                        <div class="relative sm:col-span-2" data-profile-field="direccion">
                            <label class="text-xs font-semibold text-zinc-700">Dirección</label>
                            <textarea name="direccion" rows="2" class="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-zinc-400 @error('direccion') border-amber-400 bg-amber-50 @enderror" data-profile-input="direccion">{{ old('direccion') }}</textarea>
                            @error('direccion')
                                <p class="mt-2 text-xs font-semibold text-amber-700">{{ $message }}</p>
                            @enderror
                            <p class="mt-2 hidden text-xs font-semibold text-amber-700" data-profile-error="direccion">Falta rellenar este campo.</p>
                        </div>
                        <div>
...
<script>
    (() => {
        ...
        const requiredProfileFields = ['nombres', 'apellidos', 'tipo_documento', 'numero_documento', 'telefono', 'ciudad', 'direccion'];

        const setProfileFieldState = (name, invalid, message = 'Falta rellenar este campo.') => {
            if (!pagoForm) return;
            const fieldWrap = pagoForm.querySelector(`[data-profile-field="${name}"]`);
            const input = pagoForm.querySelector(`[data-profile-input="${name}"]`);
            const errorEl = pagoForm.querySelector(`[data-profile-error="${name}"]`);

            if (fieldWrap) {
                fieldWrap.classList.toggle('rounded-2xl', invalid);
                fieldWrap.classList.toggle('bg-amber-50/60', invalid);
                fieldWrap.classList.toggle('p-2', invalid);
            }
            if (input) {
                input.classList.toggle('border-amber-400', invalid);
                input.classList.toggle('bg-amber-50', invalid);
                input.classList.toggle('shadow-[0_0_0_4px_rgba(251,191,36,0.18)]', invalid);
            }
            if (errorEl) {
        ...
        if (pagoForm) {
            requiredProfileFields.forEach((name) => {
                const input = pagoForm.querySelector(`[data-profile-input="${name}"]`);
                if (!input) return;
                input.addEventListener('input', () => {
                    if (isBlank(input.value)) {
                        setProfileFieldState(name, true);
                    } else {
                        setProfileFieldState(name, false);
                    }
                });
            ...
            pagoForm.addEventListener('submit', (event) => {
                ...
                requiredProfileFields.forEach((name) => {
                    const input = pagoForm.querySelector(`[data-profile-input="${name}"]`);
                    const invalid = !input || isBlank(input.value);
                    setProfileFieldState(name, invalid);
                    if (invalid) hasError = true;
                });

                if (hasError) {
                    event.preventDefault();
                    const firstInvalid = requiredProfileFields
                        .map((name) => pagoForm.querySelector(`[data-profile-input="${name}"]`))
                        .find((input) => !input || isBlank(input.value));
                    if (firstInvalid) {
                        try {
                            firstInvalid.focus();
